...
public:
  ...
  // Array(Array<T>&& a) noexcept : buf(nullptr), size(0) { *this =
  // std::move(a); }
...
public:
  ...
  ~Array2d(void) { delete[] buf; }
  iVector2d operator()(void) const { return size; }
  iVector2d getSize(void) const { return size; }
  iVector2d get_size(void) const { return size; }
  T &operator()(iVector2d i) { return access(i(0), i(1)); }
  T &operator()(int x, int y) { return access(x, y); }
...
public:
  ...
  void init(void) { i = iVector2d(0, 0); }
  int operator++(int) {
    i(0)++;
    if (i(0) == size(0)) {
      i(0) = 0;
  ...
  operator iVector2d(void) const { return i; }
