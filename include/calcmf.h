...
public:
  CalcMeanField(const Structure &_lattice,
                const Array<Array<int>> &_site_type_list, const Structure &_str,
  ...
  void set_ref_prob(const Array<Array<Real>> &_ref_prob) {
    ref_prob = _ref_prob;
  }
...
void calc_conc_from_phi(Array<Real> *pconc, GCThermoData *ptd, Real T,
                        const Array<Real> &mu, Real dmu);
Real calc_E_from_phi(GCThermoData *ptd, Real T, const Array<Real> &mu, Real dT);
Real calc_E_from_phi(CalcMeanField *ptd,
                     const PolyInterpolatorBig<Array<Real>> &teci, Real T,
                     const Array<Real> &mu, Real dT);
