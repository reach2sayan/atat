...
public:
  ...
  T *add(T *newobj, LinkedListIterator<T> &insertat) {
    if (insertat.previous) {
      insertat.previous->next =
          new LinkedListNode<T>(newobj, insertat.previous->next);
  ...
  int get_size(void) const { return getSize(); }
...
protected:
  ...
  LinkedListNode<T> *get_current(void) {
    return (previous ? previous->next : NULL);
  }

public:
  LinkedListIterator(void) { previous = NULL; }
  LinkedListIterator(const LinkedList<T> &linkedlist) { init(linkedlist); }
  void init(const LinkedList<T> &linkedlist) { previous = linkedlist.head; }
  ...
  T &operator*(void) { return *(get_current()->obj); }
  operator T *() {
    if (get_current())
...
template <class T> void transfer_list(LinkedList<T> *dest, LinkedList<T> *src) {
  LinkedListIterator<T> i(*src);
  while (i) {
    (*dest) << src->detach(i);
  }
...
template <class T> inline void add_at_beginning(LinkedList<T> *ll, T *obj) {
  LinkedListIterator<T> beginning(*ll);
  ll->add(obj, beginning);
}

template <class T> ostream &operator<<(ostream &s, const LinkedList<T> &a) {
  s << a.get_size() << endl;
...
template <class T> istream &operator>>(istream &s, LinkedList<T> &a) {
  int size = 0;
  s >> size;
