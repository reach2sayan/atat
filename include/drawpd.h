...
public:
  ...
  virtual const char *object_label(void) { return ""; }
  ...
  Real phi(Real T, Real mu) {
    set_T_mu(T, mu);
    return cur_phi;
  }
  Real x(Real T, Real mu) {
    set_T_mu(T, mu);
    return cur_x;
  }
  PhaseThermoData *get_sub_phase(Real T, Real mu) {
    set_T_mu(T, mu);
    return cur_phase;
  }
};
...
public:
  ...
  const char *object_label(void) { return "MF"; }
...
public:
  ...
  void read(istream &file);
  void write(ostream &file);
  const char *object_label(void) { return "LTE"; }
...
public:
  HTE_ThermoData(void);
  HTE_ThermoData(const Structure &lattice, const SpaceGroup &space_group,
                 const Structure &str,
                 const LinkedList<ArrayrVector3d> &cluster_list,
                 const LinkedList<Real> &eci_list);
  void init(const Structure &lattice, const SpaceGroup &space_group,
  ...
  void read(istream &file);
  void write(ostream &file);
  const char *object_label(void) { return "HTE"; }
...
public:
  ...
  SampledThermoData(Real _T0, Real _dT, const Array<Real> &_mu0, Real _dmu,
                    const Array<ArrayReal> &_xs, const Array<ArrayReal> &_phis);
  void set_T_mu(Real T, Real mu);
  void read(istream &file);
  void write(ostream &file);
  const char *object_label(void) { return "TAB"; }
...
public:
  ...
  void read(istream &file);
  void write(ostream &file);
  const char *object_label(void) { return "lstT"; }
...
public:
  ...
  void read(istream &file);
  void write(ostream &file);
  const char *object_label(void) { return "lst"; }
...
public:
  MiscibilityGap(void);
  MiscibilityGap(PhaseThermoData *_hi, PhaseThermoData *_lo0,
                 PhaseThermoData *_lo1);
  ~MiscibilityGap();
  void set_T_mu(Real T, Real mu);
  void read(istream &file);
  void write(ostream &file);
  const char *object_label(void) { return "MG"; }
...
void recurse_phase_boundaries(LinkedList<PhaseBoundary> *pb_list,
                              const LinkedList<PhaseThermoData> &phases, Real T,
                              Real mu0, Real mu1, Real dx);
void calc_phase_boundaries(LinkedList<PhaseBoundary> *pb_list,
                           const LinkedList<PhaseThermoData> &phases, Real T,
                           Real dx, const Array<Real> &mu_hints);
int same_phases(const LinkedList<PhaseBoundary> &pb_list1,
                const LinkedList<PhaseBoundary> &pb_list2);
void add_one_T(LinkedList<TwoPhaseRegion> *two_phase_list, Real T,
               const LinkedList<PhaseBoundary> &pb_list);

void calc_phase_boundaries(LinkedList<TwoPhaseRegion> *two_phase_list,
                           const LinkedList<PhaseThermoData> &phases, Real dT,
                           Real dx, const Array<Real> &mu_hints);
void write_phase_diagram(const LinkedList<TwoPhaseRegion> &two_phase_list,
                         ostream &file);
