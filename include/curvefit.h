...
{
    FitData* d  = static_cast<FitData*>(params);
...
template<typename Callable>
Array<double> curve_fit_impl(func_f_type f, func_df_type df, func_fvv_type fvv, gsl_vector* initial_params, fit_data<Callable>& fd) {
...
Array<double> curve_fit(Callable f, const Array<double>& initial_params, const Array<double>& x, const Array<double>& y) {
    ...
    constexpr size_t n = decltype(n_params(std::function<Callable>(f)))::n_args - 1;
