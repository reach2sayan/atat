...
public:
  ...
  SideFlag(int _done, const Array<int> &_ipts)
      : done(_done), ipts(_ipts), forward(0), normal(0) {}
};
...
template <class T> class OrderBy_r {
public:
  int operator()(const T &a, const T &b) const { return (a.r < b.r); }
};
...
public:
  PointFromCenterIteratorGridIndex(const Array<Array<Real>> &_pts,
                                   const PointGridIndex &_grid_index,
...
public:
  PointFromCenterIterator(const Array<Array<Real>> &_pts,
                          const Array<Real> &_center);
  void init(void);
  operator void *();
  int operator()(void);
...
void cross_section_simplexes(
    ...
    Array<Array<Real>> *pcurcolors, const Array<Array<Real>> &axes,
