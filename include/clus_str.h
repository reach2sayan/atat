...
Real get_length_slow(const Array<rVector3d> &c);
...
public:
  ClusterBank(const rMatrix3d &cell, const Array<rVector3d> &atom_pos,
              int ntuple, const SpaceGroup &_equivalent_by_symmetry);
  ClusterBank(const ClusterBank &clusterbank, int ntuple);
  void reset(void);
  void operator++(int);
  int get_current_index(void) { return current_index; }
  Real get_previous_length(void) { return previous_length; }
  Real get_current_length(void) { return get_length_quick(*current_cluster); }
  operator Cluster &(void) { return *current_cluster; }
  LinkedList<Cluster> &get_cluster_list(void) { return cluster_list; }
};
...
public:
  MultiClusterBank(const Structure &_lat, int ntuple,
  ...
  operator MultiCluster &(void) { return *current_cluster; }
  MultiCluster *operator->() { return current_cluster; }
  LinkedList<MultiCluster> &get_cluster_list(void) { return cluster_list; }
};

// The best algorithm is invoked by not specifying any -D... in the makefile;
// If you want to use MPI, use -DSLOWENUMALGO ;
// For debugging, you can use the slowest algorithm with -DSLOWENUMALGO
// -DOLD_STR_ALGO ;

#ifndef SLOWENUMALGO
// This is the best algorithm;
template <class T> class StructureBank {
  int current_index;
  const SpaceGroup &equivalent_by_symmetry;
  ...
  void init(void) {
    ...
    find_all_atom_in_supercell(
        &blank_superstructure.atom_pos, &blank_superstructure.atom_type,
        basic_structure.atom_pos, basic_structure.atom_type,
    ...
    for (int t = 0; t < max(savetype); t++) {
      ...
      for (; i; i++) {
        if (equivalent_by_symmetry(*i, blank_superstructure))
          break;
      }
      if (!i) {
...
public:
  StructureBank(const Structure &_basic_structure,
      ...
      : structure_list(), current_structure(),
        ...
        basic_structure(_basic_structure), do2D(_do2D), supercells(),
        curnew_config(), beginning_of_cell(), subgroup() {
    init();
  }
  StructureBank(const Structure &_basic_structure,
    ...
    for (int i = 0; i < basic_structure.atom_type.get_size(); i++) {
      basic_structure.atom_type(i) =
          site_type_list(basic_structure.atom_type(i)).get_size();
    }
    init();
  }
  void set_2D_mode(int _do2D) { do2D = _do2D; }
  void reset(void) {
    current_index = 0;
  ...
  LinkedList<T> &get_structure_list(void) { return structure_list; }
  int get_current_index(void) { return current_index; }
  int get_max_volume(void) { return current_volume; }
  /*
  int end_of_this_volume(void) { // caution: this routine only works if
  add_structure is not called. if (supercells.get_size()==0) {return 0;} if
  (curnew_config) {return 0;} else {return 1;}
  }
  */
  ...
  int add_structure(const Structure &str, T **pp_str = NULL) {
    LinkedListIterator<T> i(structure_list);
    for (; i; i++) {
      if (equivalent_by_symmetry(*i, str))
        break;
    }
    if (!i) {
      LinkedListIterator<T> insert_at(structure_list);
      while (insert_at &&
             insert_at->atom_pos.get_size() < str.atom_pos.get_size() + 1)
        insert_at++;
      T *p_str = new T(str);
...
template <class T> int StructureBank<T>::find_new_structure(void) {
  while (1) {
    if (!curnew_config) {
      curnew_supercell++;
      if (curnew_supercell >= supercells.get_size()) {
        ...
        pointgroup_from_spacegroup(&pointgroup,
                                   equivalent_by_symmetry.point_op);
        if (do2D) {
          find_supercells_2D(&supercells, current_volume, current_volume,
                             basic_structure.cell, pointgroup);
        } else {
          find_supercells(&supercells, current_volume, current_volume,
                          basic_structure.cell, pointgroup);
        }
        curnew_supercell = 0;
        int target_nbatom =
            basic_structure.atom_pos.get_size() * current_volume;
        beginning_of_cell.init(structure_list);
        // cerr << "begincell [\n";
        while (1) {
          if (!beginning_of_cell)
            break;
          if (beginning_of_cell->atom_pos.get_size() >= target_nbatom)
            break;
          beginning_of_cell++;
        }
        // cerr << "begincell ]\n";
      ...
      // cerr << "findsymcell [\n";
      supercells(curnew_supercell) =
          find_symmetric_cell(supercells(curnew_supercell));
      // cerr << "findsymcell ]\n";
      ...
      subfactorgroup_from_supercell_and_spacegroup(
          &subgroup.point_op, &subgroup.trans, blank_superstructure.cell,
          equivalent_by_symmetry.point_op, equivalent_by_symmetry.trans);
    ...
    if (!current_structure) {
      need_to_gen = 1;
    } else if (current_structure->atom_pos.get_size() /
                   basic_structure.atom_pos.get_size() >
               MAX(1, current_volume)) {
      need_to_gen = 1;
    }
    if (!need_to_gen)
      return 0;
    ...
    curnew_config++;
#ifdef ENUMFLIPTRICK
    if (!contains_pure_translations(blank_superstructure,
                                    basic_structure.cell)) {
      if (!equiv_to_lexico_successor(blank_superstructure, basic_structure.cell,
                                     subgroup.point_op, subgroup.trans)) {
        if (!useradded && current_volume > 1) {
...
#else
    if (!contains_pure_translations_or_lexico_successor(blank_superstructure,
                                                        basic_structure.cell)) {
      LinkedListIterator<T> i = beginning_of_cell;
      for (; i; i++) {
        if (equivalent_by_symmetry(*i, blank_superstructure))
...
public:
  ...
  operator T &(void) { return *current_structure; }
  T &get_current_structure(void) { return *current_structure; }
  LinkedList<T> &get_structure_list(void) { return structure_list; }
  int get_current_index(void) { return current_index; }
  int get_max_volume(void) { return current_volume; }
  int end_of_this_volume(void) {
    ...
    if (!tmp_structure)
      return 1;
    if (tmp_structure->atom_pos.get_size() /
            basic_structure.atom_pos.get_size() >
        current_volume)
      return 1;
    return 0;
...
void StructureBank<T>::find_new_structures(int new_max_volume) {
  ...
  while (insert_at && insert_at->atom_pos.get_size() <= nb_atom)
    insert_at++;
  ...
  if (do2D) {
    find_supercells_2D(&supercell, current_volume + 1, new_max_volume,
                       basic_structure.cell, pointgroup);
  } else {
    find_supercells(&supercell, current_volume + 1, new_max_volume,
                    basic_structure.cell, pointgroup);
  }
    ...
    for (int c = 0; c < supercell.get_size(); c++) {
      if (sync.is_my_job()) {
        ...
        for (; config; config++) {
          ...
          if (!contains_pure_translations_or_lexico_successor(
                  blank_superstructure, basic_structure.cell)) {
        ...
        while (i) {
          ...
          int found = 0;
          while (j) {
            if (j->atom_pos.get_size() != i->atom_pos.get_size())
              break;
