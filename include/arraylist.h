...
/*
template<class T, int D>
void ArrayArray_to_ArrayFixedVector(Array<FixedVector<T,D> > *paf, const
Array<Array<T> > &aa) { paf->resize(aa.get_size()); for (int i=0;
i<aa.get_size(); i++) { Array_to_FixedVector(&((*paf)(i)),aa(i));
  }
}

template<class T, int D>
void LinkedListFixedVector_to_LinkedListArray(LinkedList<Array<T> > *pla, const
LinkedList<FixedVector<T,D> > &lf) { LinkedListIterator<FixedVector<T,D> >
it(lf); for (;it; it++) { Array<T> *pa=new Array<T>();
...
template<class T>
void LinkedListFixedVector_to_LinkedListArray(LinkedList<Array<T> > *pla, const
LinkedList<Vector3d<T> > &lf) { LinkedListIterator<Vector3d<T> > it(lf); for
(;it; it++) { Array<T> *pa=new Array<T>(); FixedVector_to_Array(pa,*it);
    (*pla) << pa;
  }
}
*/

template <class T> class TrivEqual {
public:
...
template <class T> class TrivLessThan {
public:
  int operator()(const T &a, const T &b) const { return (a < b); }
};
...
template <class T> class MultiDimIterator {
  T current;
  T max;
  T min;
  int valid;
...
public:
  ...
  MultiDimIterator(const T &_max) : current(), min(), max() { init(_max); }
...
template <class T> inline const T &MultiDimIterator<T>::operator++(int) {
  int i = 0;
...
template <class T> inline void MultiDimIterator<T>::bump_up(int level) {
...
public:
  ...
  MultipletIterator(int dim, int _min, int _max, Style _style = all)
      : current() {
...
inline const Array<int> &MultipletIterator::operator++(int) {
  ...
  case MultipletIterator::norepeat:
    current(i) = min;
    i = 0;
    while (current(i) == max && i < current.get_size() - 1)
      i++;
    current(i)++;
    for (int j = i - 1; j >= 0; j--) {
...
public:
  NTupleIterator(int _nb, int _outof) : current(0) { init(_nb, _outof); }
...
void extract_columns(Array2d<T> *pa, const Array2d<T> &b,
                     const LinkedList<int> &cols_list, int nb_cols) {
  Array<int> cols;
