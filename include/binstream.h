...
#define MAKE_GENERIC_BIN_OSTREAM(T)                                            \
  inline ostream &bin_ostream(ostream &file, const T &x) {                     \
    return file.write((char *)&x, sizeof(T));                                  \
  }
...
#define MAKE_GENERIC_BIN_ISTREAM(T)                                            \
  inline istream &bin_istream(istream &file, T &x) {                           \
    return file.read((char *)&x, sizeof(T));                                   \
  }
...
template <class T> inline istream &bin_istream(istream &file, Array<T> &x) {
  int n;
  bin_istream(file, n);
  x.resize(n);
  for (int i = 0; i < n; i++) {
