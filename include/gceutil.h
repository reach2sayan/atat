...
public:
  ...
  virtual void find_basis(LinkedList<SymmetryObeyingObject> *plistobj,
                          const SpaceGroup &space_group) {}
  virtual void zero(void) {}
  virtual SymmetryObeyingObject *clone(void) { return NULL; }
  virtual void apply_symmetry(const rMatrix3d &point_op,
                              const rVector3d &trans) {}
...
public:
  ...
  GeneralizedCluster(const GeneralizedCluster &gc) : clus(gc.clus) {
    if (!gc.psymobj) {
      psymobj = NULL;
    } else {
      psymobj = gc.psymobj->clone();
    }
  }
  void operator=(const GeneralizedCluster &gc) {
    clus = gc.clus;
    if (!gc.psymobj) {
      psymobj = NULL;
    } else {
  ...
  GeneralizedCluster(const MultiCluster &_clus, SymmetryObeyingObject *_psymobj)
      : clus(_clus) {
    psymobj = _psymobj;
  }
};
...
public:
  ...
  void find_basis(LinkedList<SymmetryObeyingObject> *plistobj,
                  const SpaceGroup &space_group);
  void zero(void);
...
void find_equivalent_clusters(Array<GeneralizedCluster> *pclusters,
                              const GeneralizedCluster &cluster,
                              const rMatrix3d &cell,
                              const Array<rMatrix3d> &point_op,
                              const Array<rVector3d> &trans);
int calc_multiplicity(const GeneralizedCluster &cluster, const rMatrix3d &cell,
                      const Array<rMatrix3d> &point_op,
                      const Array<rVector3d> &trans);
void calc_correlation(SymmetryObeyingObject *gencorr, const Structure &str,
                      const Array<GeneralizedCluster> &clusters,
                      const rMatrix3d &cell,
                      const Array<Array<Array<Real>>> &corrfunc);
void calc_correlation(SymmetryObeyingObject *gencorr, const Structure &str,
                      const GeneralizedCluster &cluster, const rMatrix3d &cell,
                      const Array<rMatrix3d> &point_op,
                      const Array<rVector3d> &trans,
                      const Array<Array<Array<Real>>> &corrfunc);

Real read_clusters_and_eci(LinkedList<GeneralizedCluster> *clusterlist,
