...
public:
  ...
  MonteCarlo(const Structure &_lattice, const iVector3d &_supercell,
  ...
  Real get_cur_concentration(void) const { return cur_conc; }
  Real get_cur_disorder_param(void) const { return cur_disorder_param; }
  int get_total_clusters(void) const { return total_clusters; }
  void get_cluster_mult(Array<Real> *mult) const {
    mult->resize(total_clusters);
    for (int i = 0; i < total_clusters; i++) {
      (*mult)(i) = rcluster_mult_per_atom[i];
    }
  }
...
protected:
  ...
  virtual void extension_update_spin_flip(int *cell, int incell, int newspin,
                                          Real d_recip_energy) {}
  virtual void extension_undo_spin_flip(void) {}
  virtual Real extension_spin_flip_energy(int *cell, int incell, int newspin) {
    return 0.;
  }
...
void run_mc_until_converged(MonteCarlo *pmc, int mode, Equilibrator *eq,
                            Real prec, int which_elem = 1);
void run_mc(MonteCarlo *pmc, int mode, int n_step, Accumulator *eq);
void run_mc(MCOutputData *pmcdata, MonteCarlo *pmc, int mode, int n_step,
            Real prec, int which_elem = 1);
...
public:
  Equilibrator(void);
  Equilibrator(Real _prec, int data_size = 1, int _which_elem = 0,
               int init_granularity = 500, int nb_bin = 16);
  void init(Real _prec, int data_size = 1, int _which_elem = 0,
            int init_granularity = 500, int nb_bin = 16);
  ...
  void get_step(int *pequil, int *pstep) {
    if (pequil)
      *pequil = equil;
    if (pstep)
      *pstep = step;
...
public:
  ...
  int new_data(const Array<Real> &data);
  const Array<Real> &get_mean(void) {
    accum();
    return good_val;
  }
  const Array<Real> &get_var(void) {
    accum();
    return good_val2;
  }
};
...
class FlippedSpin {
public:
  FlippedSpin(int *_cell, int _incell, SPIN_TYPE _newspin,
...
public:
  KSpaceMonteCarlo(const Structure &_lattice, const iVector3d &_supercell,
                   const SpaceGroup &space_group,
                   const LinkedList<Cluster> &cluster_list,
                   KSpaceECI *_p_kspace_eci);
  ~KSpaceMonteCarlo();

protected:
  void set_k_space_eci(void);
  void extension_calc_from_scratch(void);
  void extension_update_spin_flip(int *cell, int incell, int newspin,
                                  Real d_recip_energy);
