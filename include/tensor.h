...
template <class T> Real norm(const Tensor<T> &t);
...
public:
  ...
  Tensor(const Array<int> &_dim) : dim(), x() { resize(_dim); }
  ...
  void operator+=(const Tensor<T> &a) { sum(&x, x, a.x); }
  void operator-=(const Tensor<T> &a) { diff(&x, x, a.x); }
  ...
  T *get_buf(void) { return x.get_buf(); }
  Array<T> &vectorize(void) { return x; }
  Array<T> const &vectorize(void) const { return x; }
};
