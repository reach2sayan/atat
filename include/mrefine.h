...
public:
  ...
  StructureInfo(const Structure &str)
      : Structure(str), label(), correlations(), predictor() {
    energy = 0;
    status = unknown;
    cost = 0.;
  }
...
public:
  ...
  //  new ground states (whose exact energy is not known yet) have been
  //  predicted;
  Status status;
  ...
  Array<int> gs_list; // indices (into any of the 4 previous arrays) of the true
                      // ground states
  ...
  Array2d<Real> conc_to_fullconc; // converts nonredundant concentration to full
                                  // redundant concentration;
  Array<Real> conc_to_fullconc_c; // constant terms of previous matrix;
  int max_volume; // largest volume considered when looking for new ground
                  // states;
...
protected:
  ...
  StructureBank<StructureInfo>
      structures; // structure generation object     \ see clus_str.h;
  Array<MultiClusterBank *>
      pclusters; // cluster generation object /;
                 // *(pclusters(0)): points, *(pclusters(1)): pairs, etc.;
  Array<LinkedList<Array<MultiCluster>> *>
      equiv_cluster;    // image of each cluster by all elements of the factor
                        // group;
  ...
  int minimal_nc; // number of cluster in a minimal nearest neighbor cluster
                  // expansion;
  ...
  int ignore_gs;
  int user_max_vol;
  Real giveup_weight; // max weight on structure before giving up;
...
public:
  int max_multiplet; // maximum number of points in cluster (can be increased
                     // later on);
  Real complexity_exp;
  ...
  // update corr. of all structures of a given status (multiple statuses can be
  // or-ed together);
  ...
  void calc_concentration(Array<Real> *pconcentration,
                          const Array<Real> &corr_matrix);
  // calculate concentration from correlation matrix;
  void calc_concentration(Array<Array<Real>> *pconcentration,
                          const Array2d<Real> &corr_matrix);
  // create a matrix of corr. and a vector of energies for structures maching a
  // given status;
  void calc_regression_matrices(
      Array2d<Real> *pcorr_matrix, Array<Real> *penergy,
      StructureInfo::Status select = StructureInfo::calculated);
  // create a vector of predictor energies (using the user-specified predictor
  // objects) for structures maching a given status;
  void calc_predictor_energy(
      Array<Real> *penergy,
      StructureInfo::Status select = StructureInfo::calculated);
  ...
  // select a given choice of clusters: choice(1) gives the nunber of pairs,
  // choice(2) gives the number of triplets, etc.;
  void set_cluster_choice(const Array<int> &choice);
  // opposite of the above;
  void get_cluster_choice(Array<int> *pchoice);
  // make an array of correlations from the list of corr. stored in a
  // StructureInfo object;
  void make_correlation_array(Array<Real> *array_corr,
                              const LinkedList<LinkedListReal> &list_corr);
  ...
  void set_concentration_range(const LinkedList<LinearInequality> &_conc_range,
                               int _ignore_gs);
  // select predictors;
  void set_predictor_labels(const char *labels);
  // set max energy above GS hull allowed when generating new structures;
  void set_highest_energy_allowed(Real _high_energy) {
    high_energy = _high_energy;
  };
  void set_maximum_volume_allowed(int _user_max_vol) {
    user_max_vol = _user_max_vol;
  };
  void set_max_weight(Real _max_weight) { giveup_weight = _max_weight; }

  // the following 4 routines can be overridden in derived class to enable new
  // fitting algorithms;
  ...
  // Main routine calling the structure enumeration routines and the variance
  // calculation codes.; Return pointer to structure found (or NULL upon
  // failure);
  ...
  // Returns the least computationally intensive structure still marked as
  // unknown. Return pointer to structure (never NULL);
  virtual StructureInfo *find_first_unknown_structure(void);

  // various functions to access private members;
  const Structure &get_lattice(void) const { return parent_lattice; }
  const Structure &get_lattice_only(void) const { return lattice_only; }
...
public:
  virtual ClusterExpansion *create(const Structure &_parent_lattice,
    ...
    return new ClusterExpansion(_parent_lattice, _site_type_list, _atom_label,
                                _spacegroup, _pcorrfunc);
  }
};

template <class T>
class CustomClusterExpansionCreator
