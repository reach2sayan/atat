...
public:
  Equilibrator(void);
  Equilibrator(Real _prec, int _which_elem = 0, int init_granularity = 200,
               int nb_bin = 16);
  void init(Real _prec, int _which_elem = 0, int init_granularity = 200,
            int nb_bin = 16);
...
public:
  Accumulator(int _max_n = 0) : cur_sum(), cur_sum2(), good_val() {
    max_n = _max_n;
    cur_n = 0;
  }
  int new_data(const Array<Real> &data);
