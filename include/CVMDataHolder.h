...
class CVMOptimizerDataHolder {
	...
	public: //logging
		...
		void addInfo(const VectorXd& optcorr, double optfe, double ordfe, double disordfe, double T);
	...
	public:
		...
		static void find_subclusters(LinkedList<MultiCluster> *pcluslist, const MultiCluster &maxclus, const SpaceGroup &spacegroup);
		static void generate_functions_on_clusters(LinkedList<MultiCluster> *pmulticluslist, const LinkedList<MultiCluster> &cluslist, const SpaceGroup &spacegroup);
		static void generate_config_on_clusters(LinkedList<LinkedList<MultiCluster> > *pconfiglistlist, LinkedList<LinkedList<double> > *pmultlistlist,
				const LinkedList<MultiCluster> &cluslist, const SpaceGroup &spacegroup);
		static void calc_v_matrix(LinkedList<Array2d<double>> *pvmatlist,
		...
		static void calc_kikuchi_barker(Array<double> *pkbcoef, const LinkedList<MultiCluster> &cluslist, const SpaceGroup &spacegroup);
		static void get_structure_from_file(Structure& str, const string& strfilename, const Structure& lattice, const Array<AutoString>& label, const Array<Arrayint>& labellookup);
