...
template <class T> T sgn(T b) {
  return (b > (T)0 ? (T)1 : (b < (T)0 ? (T)(-1) : (T)0));
}

template <class T> inline T ipow(T base, int exponent) {
  if (exponent < 0) {
...
template <class T> int is_between(T bw, T a, T b) {
...
inline Real uniform01(void) { return (Real)(rand()) / (Real)RAND_MAX; }
