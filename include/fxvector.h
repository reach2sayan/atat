...
template <class T, int D> class FixedVector;
...
int operator!=(const FixedVector<T, D> &a, const FixedVector<T, D> &b);
template <class T, int D>
FixedVector<T, D> operator-(const FixedVector<T, D> &a,
                            const FixedVector<T, D> &b);
template <class T, int D>
FixedVector<T, D> operator+(const FixedVector<T, D> &a,
                            const FixedVector<T, D> &b);
template <class T, int D>
...
template <class T, int D> T norm2(const FixedVector<T, D> &a);
template <class T, int D> Real norm(const FixedVector<T, D> &a);
template <class T, int D> T max_norm(const FixedVector<T, D> &a);
template <class T, int D> T l1_norm(const FixedVector<T, D> &a);

template <class T, int D> class FixedVector {
  friend int operator==
      <>(const FixedVector<T, D> &a, const FixedVector<T, D> &b);
  friend int operator!=
      <>(const FixedVector<T, D> &a, const FixedVector<T, D> &b);
  friend FixedVector<T, D> operator-
      <>(const FixedVector<T, D> &a, const FixedVector<T, D> &b);
  friend FixedVector<T, D> operator+
      <>(const FixedVector<T, D> &a, const FixedVector<T, D> &b);
...
template <class T> class Vector2d : public FixedVector<T, 2> {
...
template <class T> class Vector3d : public FixedVector<T, 3> {
  friend Vector3d<T> operator^
      <>(const FixedVector<T, 3> &a, const FixedVector<T, 3> &b);
...
template <class T>
Vector3d<T> operator^(const FixedVector<T, 3> &a, const FixedVector<T, 3> &b) {
  return Vector3d<T>(a.x[1] * b.x[2] - a.x[2] * b.x[1],
...
template <class T> istream &operator>>(istream &s, Vector2d<T> &v) {
  s >> v.x[0] >> v.x[1];
  return s;
}

template <class T> istream &operator>>(istream &s, Vector3d<T> &v) {
  s >> v.x[0] >> v.x[1] >> v.x[2];
  return s;
}

template <class T> ostream &operator<<(ostream &s, const FixedVector<T, 2> &v) {
  s << v.x[0] << " " << v.x[1];
  return s;
}

template <class T> ostream &operator<<(ostream &s, const FixedVector<T, 3> &v) {
  s << v.x[0] << " " << v.x[1] << " " << v.x[2];
  return s;
}
...
template <class T, int D> class FixedMatrix;

template <class T, int D> T max_norm(const FixedMatrix<T, D> &a);
template <class T, int D> Real norm(const FixedMatrix<T, D> &a);
template <class T, int D>
FixedMatrix<T, D> operator-(const FixedMatrix<T, D> &a,
                            const FixedMatrix<T, D> &b);
template <class T, int D>
FixedMatrix<T, D> operator+(const FixedMatrix<T, D> &a,
                            const FixedMatrix<T, D> &b);
template <class T, int D>
...
FixedMatrix<T, D> operator*(const FixedMatrix<T, D> &a,
                            const FixedMatrix<T, D> &b);
template <class T, int D>
FixedVector<T, D> operator*(const FixedMatrix<T, D> &m,
                            const FixedVector<T, D> &v);
template <class T, int D>
FixedMatrix<T, D> operator~(const FixedMatrix<T, D> &a);
template <class T, int D> T trace(const FixedMatrix<T, D> &a);
template <class T, int D>
...
template <class T, int D> class FixedMatrix {
  ...
  friend FixedMatrix<T, D> operator-
      <>(const FixedMatrix<T, D> &a, const FixedMatrix<T, D> &b);
  friend FixedMatrix<T, D> operator+
      <>(const FixedMatrix<T, D> &a, const FixedMatrix<T, D> &b);
  ...
  friend FixedMatrix<T, D> operator*
      <>(const FixedMatrix<T, D> &a, const FixedMatrix<T, D> &b);
  friend FixedVector<T, D> operator*
      <>(const FixedMatrix<T, D> &m, const FixedVector<T, D> &v);
...
FixedMatrix<T, D> operator*(const FixedMatrix<T, D> &a,
                            const FixedMatrix<T, D> &b) {
  RESULT_FORALLij(result.x[i][j] = (T)0; for (int k = 0; k < D; k++) {
...
template <class T> T det(const FixedMatrix<T, 2> &a);
template <class T> Matrix2d<T> operator!(const FixedMatrix<T, 2> &a);

template <class T> class Matrix2d : public FixedMatrix<T, 2> {
...
template <class T> T det(const FixedMatrix<T, 3> &a);
template <class T> FixedMatrix<T, 3> operator!(const FixedMatrix<T, 3> &a);

template <class T> class Matrix3d : public FixedMatrix<T, 3> {
...
template <class T> T det(const FixedMatrix<T, 3> &a) {
  return a.x[0][0] * (a.x[1][1] * a.x[2][2] - a.x[1][2] * a.x[2][1]) +
...
template <class T> ostream &operator<<(ostream &s, const FixedMatrix<T, 2> &a) {
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      s << a.x[i][j] << " ";
    }
...
template <class T> ostream &operator<<(ostream &s, const FixedMatrix<T, 3> &a) {
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      s << a.x[i][j] << " ";
    }
...
template <class T> istream &operator>>(istream &s, Matrix2d<T> &a) {
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      s >> a.x[i][j];
    }
...
template <class T> istream &operator>>(istream &s, Matrix3d<T> &a) {
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      s >> a.x[i][j];
    }
...
template <class T, int D> class BoundingBox {
public:
  FixedVector<T, D> min, max;

public:
  BoundingBox(void) : min(), max() {}
  BoundingBox(const FixedVector<T, D> &_min, const FixedVector<T, D> &_max)
      : min(_min), max(_max) {}
