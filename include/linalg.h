...
#ifdef DEBUG
    if (_a.get_size() < 2)
      ERRORQUIT("array size<2 in LinearInterpolator");
#endif
    a = _a;
    x0 = _x0;
    x1 = _x1;
...
template <class T> class LinearInterpolatorBig {
  Array<T> a;
  Real x0;
  Real x1;
...
template <class T> class PolyInterpolatorBig {
  Array<T> a;
  Real x0;
  Real x1;
...
public:
  ...
  void init(Real _x0, Real _x1, const Array<T> &_a) {
#ifdef DEBUG
    if (_a.get_size() < 3 && _a.get_size() != 1)
      ERRORQUIT("array size too small in PolyInterpolator");
#endif
    x0 = _x0;
    x1 = _x1;
    if (_a.get_size() == 1) {
