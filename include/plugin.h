...
template <class B> class GenericPlugIn {
protected:
  const char *label;
...
public:
  static B *create(const char *_label) {
    ...
    while (i && strcmp(i->label, _label) != 0)
      i = i->next;
    if (!i)
      return NULL;
    return i->make_new();
...
template <class B, class T> class SpecificPlugIn : public GenericPlugIn<B> {
public:
  ...
  virtual B *make_new(void) { return new T; }
};
...
template <class B> int check_plug_in(const B &dummy, const char *labels) {
