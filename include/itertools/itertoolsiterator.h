#ifndef __ITERTOOLS_ITERATOR_HPP__
#define __ITERTOOLS_ITERATOR_HPP__
...
class IteratorIterator {
 ...
 public:
  ...
  const TopIter& get() const { return sub_iter; }
  ...
  IteratorIterator& operator+=(std::ptrdiff_t n) {
    this->sub_iter += n;
    return *this;
  }

  IteratorIterator operator+(std::ptrdiff_t n) const {
    auto it = *this;
    it += n;
    return it;
  }

  friend IteratorIterator operator+(std::ptrdiff_t n, IteratorIterator it) {
    it += n;
    return it;
  }
  ...
  std::ptrdiff_t operator-(const IteratorIterator& rhs) const {
    return this->sub_iter - rhs.sub_iter;
  }

  auto operator[](std::ptrdiff_t idx) const -> decltype(*sub_iter[idx]) {
    return *sub_iter[idx];
  }
...
class IterIterWrapper {
 private:
  ...
  using const_iterator = IteratorIterator<typename Container::const_iterator>;
  ...
  struct ConstAtTypeOrVoid<
      U, std::void_t<decltype(*std::declval<const U&>().at(0))>>
  ...
  struct ConstIndexTypeOrVoid<
      U, std::void_t<decltype(*std::declval<const U&>()[0])>>
 ...
 public:
  ...
  const_reverse_iterator crend() const noexcept { return {container.rend()}; }
  ...
  Container& get() noexcept { return container; }

  const Container& get() const noexcept { return container; }
};

}  // namespace itertools

#endif	//__ITERATOR_ITERATOR_HPP__
