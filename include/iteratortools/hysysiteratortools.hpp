#ifndef __HYSYS_ITERATORTOOLSBASE_HPP__
#define __HYSYS_ITERATORTOOLSBASE_HPP__
...
namespace HysysIteratorTools
    ...
    // iterator_t<C> and const_iterator_t<C> is the type of C and const C' iterators respectively
    template <typename T>
    using iterator_t = decltype(fancy_getters::begin(std::declval<T&>()));

    template <typename Container>
    using const_iterator_t = decltype(fancy_getters::begin(std::declval<const std::remove_reference_t<Container>&>()));
    ...
    template <typename Container>
    using const_iterator_deref = decltype(*std::declval<const_iterator_t<Container>&>());

    // the type of dereferencing a const_iterator
    template <typename Container>
    using const_iterator_t_deref =  decltype(*std::declval<const_iterator_t<Container>&>());

    template <typename Container>
    using iterator_traits_deref = std::remove_reference_t<iterator_deref<Container>>;
    ...
    template <typename T>
    constexpr Boolean is_iterable_t = is_iterable_type<T>::value;

    template <typename... Ts>
    struct are_same : std::true_type {};

    template <typename T, typename U, typename... Ts>
    struct are_same<T, U, Ts...> : std::integral_constant < Boolean, std::is_same<T, U>::value && are_same<T, Ts...>::value {};
    ...
    template <typename T>
    struct ArrowOperatorType<T, std::void_t<decltype(std::declval<T&>().operator->())>>
        {
    ...
    class ArrowProxy 
        {
        public:
            template <typename I, typename = std::enable_if_t<std::is_same<T,I>, Boolean>>
    ...
    // the 'evaluators' only serve the purpose to give the current Types in declval(expr)
    template <typename... Ts>
    std::tuple<iterator_deref<Ts>...> iterator_tuple_deref_evaluator(const std::tuple<Ts...>&);

    template <typename... Ts>
    std::tuple<iterator_t<Ts>...> iterator_tuple_type_evaluator(const std::tuple<Ts...>&);

    template <typename... Ts>
    std::tuple<iterator_deref<const std::remove_reference_t<Ts>>...> const_iterator_tuple_deref_evaluator(const std::tuple<Ts...>&);

    template <typename... Ts>
    std::tuple<iterator_t<const std::remove_reference_t<Ts>>...> const_iterator_tuple_type_evaluator(const std::tuple<Ts...>&);

    // Given a tuple-type template argument, gives the type of the tuple of iterators
    // for the tuples contained types.
    template <typename TupleType>
    using iterator_tuple_t = decltype(iterator_tuple_type_evaluator(std::declval<TupleType>()));

    template <typename TupleType>
    using const_iterator_tuple_t = decltype(const_iterator_tuple_type_evaluator(std::declval<TupleType>()));
    ...
    template <typename TupleType>
    using iterator_deref_tuple = decltype(iterator_tuple_deref_evaluator(std::declval<TupleType>()));

    template <typename TupleType>
    using const_iterator_deref_tuple = decltype(const_iterator_tuple_deref_evaluator(std::declval<TupleType>()));
...
#endif //__HYSYS_ITERATORTOOLSBASE_HPP__