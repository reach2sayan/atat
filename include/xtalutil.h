...
public:
  ...
  Structure(const Structure &a)
      : cell(a.cell), atom_pos(a.atom_pos), atom_type(a.atom_type) {}
};

class MultiCluster {
public:
  MultiCluster(int n = 0) : clus(n), site_type(n), func(n) {}
  MultiCluster(const MultiCluster &c)
      : clus(c.clus), site_type(c.site_type), func(c.func) {}
...
inline Real cylinder_norm(rVector3d v) {
  return sqrt(sqr(cylinder_norm(v(0))) + sqr(cylinder_norm(v(1))) +
              sqr(cylinder_norm(v(2))));
}
...
inline rVector3d cylinder(rVector3d v) {
  return (
      rVector3d(cylinder_norm(v(0)), cylinder_norm(v(1)), cylinder_norm(v(2))));
}

inline int is_int(const rVector3d &v) {
  for (int i = 0; i < 3; i++) {
    if (fabs(v(i) - rint(v(i))) > zero_tolerance)
      return 0;
  }
  return 1;
}
...
inline int is_int(const rMatrix3d &m) {
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      if (fabs(m(i, j) - rint(m(i, j))) > zero_tolerance)
        return 0;
    }
  }
  return 1;
...
int equivalent_mod_cell(const Array<rVector3d> &a, const Array<rVector3d> &b,
                        const rMatrix3d &inv_cell, rVector3d *pshift = NULL);
...
public:
  ...
  LatticePointIterator(const rMatrix3d &_cell, int skipzero = 1)
      : list(), cell() {
    init(_cell, skipzero);
  }
  LatticePointIterator(const LatticePointIterator &lp) : list(), cell(lp.cell) {
    max_radius = lp.max_radius;
    inc_radius = lp.inc_radius;
    add_copy(&list, lp.list);
  }
...
public:
  ...
  LatticePointInCellIterator(const rMatrix3d &_cell,
  ...
  operator void *() { return current; }
  operator FixedVector<Real, 3>() { return cell * to_real(current); }
  void operator++(int) {
    current++;
    advance_to_valid();
  }
...
inline Real operator*(LatticePointIterator &_p, const rVector3d &_a) {
  return _a * (rVector3d)_p;
}
inline Real operator*(const rVector3d &_a, LatticePointIterator &_p) {
  return (rVector3d)_p * _a;
}
inline rVector3d operator*(const rMatrix3d &_a, LatticePointIterator &_p) {
  return _a * (rVector3d)_p;
}
inline rVector3d operator^(const rVector3d &_a, LatticePointIterator &_p) {
  return _a ^ (rVector3d)_p;
}
inline rVector3d operator^(LatticePointIterator &_p, const rVector3d &_a) {
  return (rVector3d)_p ^ _a;
}
inline Real operator*(LatticePointIterator &_a, LatticePointIterator &_p) {
  return (rVector3d)_a * (rVector3d)_p;
}

inline rVector3d operator+(const rVector3d &_a,
                           LatticePointInCellIterator &_p) {
  return _a + (rVector3d)_p;
}
inline rVector3d operator+(LatticePointInCellIterator &_p,
                           const rVector3d &_a) {
  return _a + (rVector3d)_p;
}
...
public:
  ...
  AtomPairIterator(const rMatrix3d &_cell, const Array<rVector3d> &_atom_pos1,
                   const Array<rVector3d> &_atom_pos2)
      : list(), an_atom() {
    init(_cell, _atom_pos1, _atom_pos2);
  }
  AtomPairIterator(const rMatrix3d &_cell, const rVector3d &_an_atom,
  ...
  void init(const rMatrix3d &_cell, const Array<rVector3d> &_atom_pos1,
            const Array<rVector3d> &_atom_pos2);
  void init(const rMatrix3d &_cell, const rVector3d &_an_atom,
            const Array<rVector3d> &_atom_pos);
  void operator++(int);
...
public:
  AtomMultipletIterator(const rMatrix3d &_cell,
                        const Array<rVector3d> &_atom_pos, int _ntuple)
      : pairs(), multiplet() {
  ...
  void init(const rMatrix3d &_cell, const Array<rVector3d> &_atom_pos,
            int _ntuple);
  void operator++(int);
  operator const Array<rVector3d> &(void) { return multiplet; }
  const FixedVector<Real, 3> &operator()(int j) { return multiplet(j); }
  Real length(void) { return norm(multiplet(ntuple - 1) - multiplet(0)); }
};
...
void reorder_atoms(Structure *rel_str, const Structure &str,
                   Array<int> *pcopy_from = NULL,
                   Array<iVector3d> *pshift = NULL);
