...
int lu_decomposition(Array2d<Real> *pa, Array<int> *ppivot) {
  ...
  for (int i = 0; i < n; i++) {
    Real maxel = 0.;
    for (int j = 0; j < n; j++) {
      maxel = MAX(maxel, fabs(a(i, j)));
    }
    if (maxel == 0.0) {
      if (crash_if_singular) {
...
int solve_linsys(Array2d<Real> *pa, Array<Real> *pb) {
  ...
  lapack_int INFO =
      LAPACKE_dgetrs(LAPACK_COL_MAJOR, TRANS, N, NRHS, pA, LDA, pIPIV, pB, LDB);
  if (INFO) {
    if (crash_if_singular) {
      ERRORQUITDUMP("Singular Matrix in solve_linsys!");
    }
    return 0;
  }
  return 1;
...
#elif defined(ATATUSEEIGEN)

#else
int solve_linsys(Array2d<Real> *pa, Array<Real> *pb) {
  Array<int> pivot;
  if (!lu_decomposition(pa, &pivot)) {
    return 0;
  }
...
int invert_matrix(Array2d<Real> *inv, const Array2d<Real> &a) {
  ...
  Array<int> pivot;
  if (!lu_decomposition(&linear_sys, &pivot)) {
    return 0;
  }
  Array<Real> rhs(n);
...
void tridiag_to_diag(Array<Real> *pd, Array<Real> *pe, Array2d<Real> *pa,
		     int do_vect) {
  Array<Real> &d = *pd;
  int n = d.get_size();
  Array<Real> &e = *pe;
...
void diagonalize_symmetric_matrix(Array<Real> *p_lambda, Array2d<Real> *p_vect,
  ...
  if (mat.get_size()(0) != mat.get_size()(1))
    ERRORQUIT("Matrix not square in diagonalize_symmetric_matrix");
#endif
