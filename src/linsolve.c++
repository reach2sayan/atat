...
int lu_decomposition(Array2d<Real> *pa, Array<int> *ppivot) {
  ...
  for (int i=0; i<n; i++) {
    ...
    if (maxel==0.0) {
      if (crash_if_singular) {ERRORQUITDUMP("Singular matrix in lu_decomposition!");}
      return 0;
    }
    w(i)=1./maxel;
  }
...
int solve_linsys(Array2d<Real> *pa, Array<Real> *pb) {
  ...
  if (INFO) {
    if (crash_if_singular) {ERRORQUITDUMP("Singular Matrix in solve_linsys!");}
    return 0;
  }
  return 1;
}
...
int solve_linsys(Array2d<Real> *pa, Array<Real> *pb) {
  Array<int> pivot;
  if (!lu_decomposition(pa,&pivot)) {return 0;}
  lu_backsub(pa,&pivot,pb);
  return 1;
}
#endif

int solve_linsys(Array<Real> *psoln, const Array2d<Real> &mat, const Array<Real> &vect) {
  int n=vect.get_size();
  Array2d<Real> a(mat);
  Array<Real> b(vect);
  if (!solve_linsys(&a,&b)) {return 0;}
  (*psoln)=b;
  return 1;
}
...
int invert_matrix(Array2d<Real> *inv, const Array2d<Real> &a) {
  ...
  Array<int> pivot;
  if (!lu_decomposition(&linear_sys,&pivot)) {return 0;}
  Array<Real> rhs(n);
