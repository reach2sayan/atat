...
int main(int argc, char *argv[]) {
  ...
  AskStruct options[] = {
      ...
      {"", " Scales a lattice so that nearest neighbor atom just touch.",
       TITLEVAL, NULL},
      {"-l",
       "Input file defining the lattice (default: lat.in) Use filename - to "
       "specify stdin",
       STRINGVAL, &latticefilename},
      {"-r", "Input file defining the atomic radii (default: rad.in)",
       STRINGVAL, &radiifilename},
  ...
  if (strcmp(latticefilename, "-") == 0) {
    parse_lattice_file(&lat.cell, &lat.atom_pos, &lat.atom_type,
                       &site_type_list, &atom_label, cin, &axes);
  } else {
  ...
  Array<Real> radii;
  {
    ...
    if (!file)
      ERRORQUIT("Unable to open atomic radii file.");
    while (!file.eof()) {
  ...
  for (int i = 0; i < site_r.get_size(); i++) {
    ...
    for (int j = 0; j < site_type_list(lat.atom_type(i)).get_size(); j++) {
      ...
      if (l == radii.get_size()) {
        cerr << "Don't know radius of atom "
             << atom_label(site_type_list(lat.atom_type(i))(j)) << endl;
        ERRORQUIT("Exiting.");
      }
