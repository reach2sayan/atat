...
int main(int argc, char *argv[]) {
  ...
  rVector3d mesh;
  if (setz1) {
    normalizer=pow(nb_kpts/(proj(0)*proj(1)),1./2.);
    mesh=proj*normalizer;
    mesh(2)=1.;
  }
  else {
    normalizer=pow(nb_kpts/(proj(0)*proj(1)*proj(2)),1./3.);
    mesh=proj*normalizer;
  }
  if (round) {
