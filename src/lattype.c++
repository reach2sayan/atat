...
rMatrix3d shorten_lattice_vectors(const rMatrix3d &cell) {
  rVector3d lp[3];
  for (int i = 0; i < 3; i++) {
    lp[i] = cell.get_column(i);
  }
  for (int i = 0; i < 3; i++) {
    for (int j = 1; j < 3; j++) {
      for (int s = -1; s <= 1; s += 2) {
        while (norm2(lp[i] - (lp[(i + j) % 3] * (Real)s)) < norm2(lp[i])) {
          lp[i] = lp[i] - (lp[(i + j) % 3] * (Real)s);
        }
      }
    }
  }
...
rMatrix3d find_reduced_cell(const rMatrix3d &cell) {
  // cout << cell << endl;
  if (is_reduced_cell(cell)) {
    return cell;
  }
  rMatrix3d scell = cell;
  // rMatrix3d scell=shorten_lattice_vectors(cell);
  Real d = fabs(det(scell));
  ...
  while (near_zero(norm(a ^ (rVector3d)lp[2]))) {
    lp[2]++;
  }
  rVector3d b = (rVector3d)lp[2];
  while (near_zero((b ^ a) * (rVector3d)lp[2])) {
    lp[2]++;
  }
  Real lc = norm((rVector3d)lp[2]);
  if (near_zero(norm(b) - lc)) {
    lp[2].init(scell, 1);
    while (norm((rVector3d)lp[2]) < lc - zero_tolerance) {
      lp[2]++;
    }
  }
  ...
  while (!done) {
    ...
    while (norm2(lp[1]) <= norm2(lp[2]) + zero_tolerance && !done) {
      ...
      while (norm2(lp[0]) <= norm2(lp[1]) + zero_tolerance && !done) {
        ...
        // cout << ((rVector3d)lp[2]) << " : " << ((rVector3d)lp[1]) << " : " <<
        // ((rVector3d)lp[0]); cout << " d=" << fabs(det(curcell)) << endl;
        if (near_zero(fabs(det(curcell)) - d)) {
          if (is_reduced_cell(curcell))
            done = 1;
        }
        lp[0]++;
      }
...
int is_reduced_cell(const rMatrix3d &cell) {
  ...
  if (!(aa <= bb + tiny && bb <= cc + tiny))
    return 0;
  if (!(fabs(bc) <= bb / 2. + tiny && fabs(ac) <= aa / 2. + tiny &&
        fabs(ab) <= aa / 2. + tiny))
    return 0;
  if (bc > tiny && ac > tiny && ab > tiny) { // Type I cell;
    ...
    if (!(fabs(bc) + fabs(ac) + fabs(ab) <= (aa + bb) / 2. + tiny))
      return 0;
    if (near_zero(aa - bb)) {
      if (!(fabs(bc) <= fabs(ac) + tiny))
        return 0;
    }
    if (near_zero(bb - cc)) {
      if (!(fabs(ac) <= fabs(ab) + tiny))
        return 0;
    }
...
BravaisType find_bravais_type(rMatrix3d *p_conventional,
  ...
  int i;
  for (i = 0; i < 44; i++) {
    if (i >= 0 && i <= 7 && !(near_zero(A - B) && near_zero(B - C)))
      continue;
    if (i >= 8 && i <= 16 && !near_zero(A - B))
      continue;
    if (i >= 17 && i <= 24 && !near_zero(B - C))
      continue;
    if (near_zero(norm(v[i] - DEF)) && celltype == ct_table[i]) {
      if (i == 5 || i == 6 || i == 7 || i == 14 || i == 16 || i == 23 ||
          i == 42) {
        if (near_zero(2 * fabs(D + E + F) - (A + B))) {
          if (i == 42) {
            if (near_zero(fabs(2 * D + F) - B))
              break;
          } else {
            break;
          }
...
rMatrix3d find_symmetric_cell(const rMatrix3d &cell) {
  Real transfo[][9] = {{1., 0., 0., 0., 1., 0., 0., 0., 1.},
...
/*
void find_smallest_supercell_enclosing_sphere(rMatrix3d *psupercell, const
rMatrix3d cell, Real r) { rVector3d v[3]; LatticePointIterator l(cell); while
(norm(l)<r-zero_tolerance) l++; v[0]=l; while ( norm(v[0]^l)<zero_tolerance
...
void find_smallest_supercell_enclosing_sphere(rMatrix3d *psupercell,
  ...
  while (1) {
    if (norm(v[0] ^ ll) > zero_tolerance) {
      ...
      if (fabs(proj) < (0.5 + zero_tolerance)) {
        if (curlen == 0.) {
          curlen = norm(ll);
        } else if (norm(ll) > curlen + zero_tolerance) {
          break;
        }
        if (proj > bestproj) {
          bestproj = proj;
  ...
  l++;
  while (1) {
    while ((v[0] ^ v[1]) * l < zero_tolerance || norm(l) < r - zero_tolerance)
      l++;
    MultiDimIterator<iVector2d> p(iVector2d(-1, -1), iVector2d(1, 1));
    for (; p; p++) {
      rVector2d r = to_real(p);
      rVector3d u = r(0) * v[0] + r(1) * v[1];
      if (fabs(l * u / norm2(u)) > 0.5 + zero_tolerance)
        break;
    }
    if (!p)
      break;
