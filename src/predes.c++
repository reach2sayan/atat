...
ElectroStaticPredictor::ElectroStaticPredictor(void) {
  done = 0;
  es_energy = 0.;
}
...
void ElectroStaticPredictor::static_init(const ClusterExpansion &ce) {
  ...
  ElectroStaticPredictor::species_charges.resize(
      ce.get_atom_label().get_size());
  {
    ifstream file("es.in");
...
Real ElectroStaticPredictor::get_energy(const StructureInfo &str) {
  if (!done) {
    ...
    for (int i = 0; i < str.atom_pos.get_size(); i++) {
      ...
      charges(i) = ElectroStaticPredictor::species_charges(
	  ElectroStaticPredictor::site_type_list(
	      ElectroStaticPredictor::lattice.atom_type(inlat))(
	      str.atom_type(i)));
	...
	(total_charge == 0.
	     ? 0.
	     : total_charge /
		   (Real)(total_charge > 0. ? num_positive : num_negative));
    for (int i = 0; i < str.atom_pos.get_size(); i++) {
      if (sgn(charges(i)) == sgn(total_charge)) {
	charges(i) -= d_charge;
      }
    }
    es_energy = calc_ewald(str.cell, str.atom_pos, charges) *
		(Real)(lattice.atom_pos.get_size()) /
		(Real)(str.atom_pos.get_size());
    done = 1;
  }
  return ElectroStaticPredictor::scrn * es_energy;
}

SpecificPlugIn<EnergyPredictor, ElectroStaticPredictor>
    ElectroStaticPredictorRegister("es");  // register plug-in under name "es";
