...
void list_nn_forcek_new(LinkedList<BondForcekData> *p_forcek_data, const Structure &str_ideal, const Structure &str_unpert, const Structure &str_pert) {
  ...
  while (get_length_quick(pair)<=r) {
    ...
    if (!i_bnd) {
      //      cerr << "bfd: " << bfd.atom_type[0] << " " << bfd.atom_type[1] << " " << bfd.len << " " << bfd.forcek << endl;
      (*p_forcek_data) << new BondForcekData(bfd);
    }
    pair++;
  }
...
int main(int argc, char *argv[]) {
  ...
  const char *strnamefilename="strname.in";
  ...
  if (!fitfc) {
    ...
    LinkedList<AutoString> dir_list;
    {
      ...
      while (skip_delim(strnamefile)) {
        ...
        Structure str;
        {
          ...
          ifstream strfile(ostrfilename.str().c_str());
          if (!strfile) {
            cerr << "Unable to open " << ostrfilename.str().c_str() << endl;
            ERRORQUIT("Aborting");
          }
	  ...
	  remove_vacancies(&str,atom_label,"Va");
        ...
        Structure rel_str;
        {
          ostringstream rel_strfilename;
          rel_strfilename << dirname << '/' << "str_relax.out" << '\0';
          ifstream rel_strfile(rel_strfilename.str().c_str());
          if (!rel_strfile) {
            cerr << "Unable to open " << rel_strfilename.str().c_str() << endl;
            ERRORQUIT("Aborting");
          }
	...
	if (i_bld) {
	  ...
	  for (int sb=0; sb<i_s->forcek.get_size(); sb++) {
	    ...
	    file << "'" << line.str().c_str() << "' u 1:" << (sb==0 ? '2' : '3');
	...
	{
	  ofstream datfile(line.str().c_str(),ios::app);
