...
void CVMOptimizerDataHolder::addInfo(const VectorXd& optcorr, double optfe, double ordfe, double disordfe, double T){

	double conv = GetConvFactor(UNITTYPE); // evperatom_kJpermole / lat.atom_pos.get_size();
...
double CVMOptimizerDataHolder::GetConvFactor(const UnitType type){
	double retval = 1.;
	switch(type){
		case EVPERATOM :
...
void CVMOptimizerDataHolder::saveClusterInformation(const string& clusterfname, const string& clustermultfname, const string& configfname, const string& configmultfname, const string& vmatrixfname, const string& kbfname){
	...
	for ( ; icluster; icluster++) {
		int mult = calc_multiplicity(*icluster,
				spacegroup.cell,
				spacegroup.point_op,
				spacegroup.trans);
	...
	for ( ; icluster2; icluster2++) {
		int mult = calc_multiplicity(*icluster2,
				spacegroup.cell,
				spacegroup.point_op,
				spacegroup.trans);
...
void CVMOptimizerDataHolder::PrintOptimizationVectorsAndMatrices(){
	...
	cout << "multiplicity * eci" << endl
		<< mult_eci << endl;
	cout << "config multiplicity * kb" << endl
		<< multconfig_kb << endl;
	cout << "vmatrix" << endl
		<< vmatrix << endl;
...
void CVMOptimizerDataHolder::find_subclusters(LinkedList<MultiCluster> *pcluslist, const MultiCluster &maxclus, const SpaceGroup &spacegroup) {
	...
	for (; select; select++) {
		...
		for (; iclus; iclus++)
			if (equivalent_by_symmetry(*pclus, *iclus, spacegroup.cell,spacegroup.point_op,spacegroup.trans)) 
				break;

		if (!iclus) {
			iclus.init(*pcluslist);
			for (; iclus; iclus++) {
				if (iclus->clus.get_size() > pclus->clus.get_size() || (iclus->clus.get_size() == pclus->clus.get_size() && get_length_slow(iclus->clus) > get_length_slow(pclus->clus)))
					break;
			}
...
void CVMOptimizerDataHolder::generate_functions_on_clusters(LinkedList<MultiCluster> *pmulticluslist, const LinkedList<MultiCluster> &cluslist, const SpaceGroup &spacegroup) {
...
void CVMOptimizerDataHolder::generate_config_on_clusters(LinkedList<LinkedList<MultiCluster> > *pconfiglistlist, LinkedList<LinkedList<double> > *pmultlistlist,
...
void CVMOptimizerDataHolder::calc_v_matrix(LinkedList<Array2d<double>> *pvmatlist,
		const LinkedList<LinkedList<MultiCluster> > &configlistlist, const LinkedList<LinkedList<double> > &configmultlistlist,
		const LinkedList<MultiCluster> &multicluslist,
		const SpaceGroup &spacegroup, const CorrFuncTable &corrfunc) {
	LinkedListIterator<LinkedList<MultiCluster>> iconfiglist(configlistlist);
	LinkedListIterator<LinkedList<double>> iconfigmultlist(configmultlistlist);//

	for (; iconfiglist; iconfiglist++,iconfigmultlist++) {
		Array2d<double> *pvmat=new Array2d<double>(iconfiglist->get_size(),multicluslist.get_size());
		Array2d<double> ximat(multicluslist.get_size(),iconfiglist->get_size());// debug
		...
		for (int j=0; iclus; iclus++, j++) {
			...
			for (int i=0; iconf; iconf++, i++, iconfigmult++) {
				int mult;
				double corrmult = calc_correlation_atomcluster(iconf->clus, iconf->func, const_cast<Array<MultiCluster>&>(equiv_clusters), spacegroup.cell, corrfunc, &mult);
				(*pvmat)(i,j) = corrmult/prefac;
				ximat(j,i)=(mult==0?0:corrmult/(double)mult)*(*iconfigmult); //debug
			}
		}
		(*pvmatlist) << pvmat;
	}
...
void CVMOptimizerDataHolder::calc_kikuchi_barker(Array<double> *pkbcoef, const LinkedList<MultiCluster> &cluslist, const SpaceGroup &spacegroup) {
...
MatrixXd CVMOptimizerDataHolder::get_vmatrix() const {
	...
	for(; ivmat; ivmat++){
		...
		MatrixXd current_vmat = MatrixXd::Zero(num_configs_in_cluster, num_clusters);
		for(int i = 0; i < num_configs_in_cluster; i++)
			for(int j = 0; j < num_clusters; j++)
				current_vmat(i,j) = (*ivmat)(i,j);

		vmatrix.block(row_position,0, num_configs_in_cluster, num_clusters) = current_vmat; 
...
VectorXd CVMOptimizerDataHolder::get_mults_eci() const {
	...
	for (; icluster && ieci; icluster++, ieci++, count++) {
		mult_eci(count) = calc_multiplicity(*icluster, spacegroup.cell, spacegroup.point_op, spacegroup.trans) * (*ieci);
	}
...
int CVMOptimizerDataHolder::get_num_configs() const {
	...
	int num_configs = 0;
	for(; iconfig; iconfig++)
		num_configs += (*iconfig).get_size();

	return num_configs;
}

CVMOptimizerDataHolder::CVMOptimizerDataHolder(const string& maxclusfilename, const string& latfilename, const string& ecifilename, const string& strfilename){
	...
	if (!maxclusfile)
		ERRORQUIT("Unable to open maximal cluster file");
	...
	while (true) {
		...
		while (true) {
			...
			maxclusfile >> v;
			if (maxclusfile.eof() || v(0) == MAXFLOAT || !maxclusfile) 
				break;
	...
	if (!check_plug_in(CorrFuncTable(),corrfunc_label)) 
		ERRORQUIT("Aborting");
	CorrFuncTable *pcorrfunc_ = GenericPlugIn<CorrFuncTable>::create(corrfunc_label);
...
void CVMOptimizerDataHolder::get_structure_from_file(Structure& str, const string& strfilename, const Structure& lat, const Array<AutoString>& label, const Array<Arrayint>& labellookup){
...
VectorXd CVMOptimizerDataHolder::GetSampleCorrelation() {

	return VectorXd::Zero(0);
}

VectorXd CVMOptimizerDataHolder::GetDisorderedCorrelation(){
	...
	for ( ; ipcluster; ipcluster++) {
		if ((*ipcluster)(0).clus.get_size() > 1) 
			break;
		if ((*ipcluster)(0).clus.get_size() == 1) {
			pointcorr << new Real(calc_correlation(str, *ipcluster, spacegroup.cell, *pcorrfunc));
		}
	}
		...
		for (int s = 0; s < icluster->clus.get_size(); s++) {
			...
			while (true) {
				if (ipcluster->site_type(0) == icluster->site_type(s) && 
						...
						equivalent_by_symmetry(ipcluster->clus(0),icluster->clus(s),spacegroup.cell,spacegroup.point_op,spacegroup.trans))
					break;
				ipcluster++;
				ipcorr++;
			}
