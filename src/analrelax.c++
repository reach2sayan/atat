...
int main(int argc, char *argv[]) {
  ...
  Structure str;
  {
    ...
    if (!strfile) {
      ERRORQUIT("Unable to open unrelaxed structure");
    }
    parse_lattice_file(&str.cell, &str.atom_pos, &str.atom_type,
		       &site_type_list, &label, strfile, &axes);
  ...
  Structure str_rel;
  {
    ...
    if (!strfile) {
      ERRORQUIT("Unable to open relaxed structure");
    }
    parse_lattice_file(&str_rel.cell, &str_rel.atom_pos, &str_rel.atom_type,
		       &site_type_list, &label, strfile);
