...
int main(int argc, char *argv[]) {
  ...
  int quiet=0;
  int gsnbatom=0;
  const char *conc_file="crange.in";
  int do2D=0;
  ...
  AskStruct options[]={
    ...
    {"-mw","Maximum weight allowed when try to match ground states",REALVAL,&max_weight},
  ...
  if (MyMPIobj.is_root()) write_fit_info(fitinfo,lat,site_type_list,atom_label,axes,propnameext.str().c_str());
  ...
  while (1) {
    ...
    while (update_all_structures(&(pce->access_structure_bank()),lat,site_type_list,atom_label,propname,intensive) || MyMPIobj.file_exists("refresh")) {
      ...
      if (MyMPIobj.is_root()) write_fit_info(fitinfo,lat,site_type_list,atom_label,axes,propnameext.str().c_str()); //print results;
      if (!quiet && MyMPIobj.is_root()) cerr << "done!" << endl;
    }
