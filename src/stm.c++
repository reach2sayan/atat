...
int main(int argc, char *argv[]) {
  ...
  int roll = 0;
  AskStruct options[] = {
      {"", "STM simulator " MAPS_VERSION ", by Axel van de Walle", TITLEVAL,
       NULL},
      {"-s", "Input file defining the structure geometry (default: str.out)",
       STRINGVAL, &strfilename},
      {"-r", "Input file defining the region scanned (default: region.in)",
       STRINGVAL, &regionfilename},
      {"-h",
       "Input file defining the height range (default: calculated from input)",
       STRINGVAL, &hrangefilename},
  ...
  for (int ix = 0; ix < nx; ix++) {
    for (int iy = 0; iy < ny; iy++) {
      ...
      if (iz == (nz - 1)) {
        ...
        h(ix, iy) = interpol(density, raw(iz)(iy)(ix), raw(iz + 1)(iy)(ix),
                             (Real)(iz), (Real)(iz + 1));
      }
    }
  }
  ...
  if (!regionfile) {
    ERRORQUIT("Unable to open region file");
  }
  rVector3d v0, v1, v2;
  regionfile >> v0;
  v0 = axes * v0;
  ...
  v2 = axes * v2;
  Real step =
      sqrt(norm(cell.get_column(0) ^ cell.get_column(1)) / (Real)(nx * ny)) /
      2.;
  ...
  for (int i1 = 0; i1 < n1; i1++) {
    for (int i2 = 0; i2 < n2; i2++) {
      ...
      Real a = interpol(fx, 0.0, 1.0, h(ix % nx, iy % ny),
                        h((ix + 1) % nx, iy % ny));
      Real b = interpol(fx, 0.0, 1.0, h(ix % nx, (iy + 1) % ny),
                        h((ix + 1) % nx, (iy + 1) % ny));
  ...
  if (strlen(hrangefilename) > 0) {
    ...
    if (!hfile) {
      ERRORQUIT("Unable to open height file");
    }
    hfile >> minh >> maxh;
  }
  {
