...
template <> GenericPlugIn<KSpaceECI> *GenericPlugIn<KSpaceECI>::list=NULL;
...
int main(int argc, char *argv[]) {
  ...
  int sigdig=6;
  int quiet=0;
  const char *outfile="mc.out";
  const char *snapshotfile="mcsnapshot.out";
  const char *snapshotnumfile="";
  Real kboltzman=1.;
  int keV=0;
  int seed=0;
  int droplast=0;
  ...
  int half_shift=0;
  int include_last=0;
  const char *conc_axes="";
  const char *corrfunc_label="trigo";
  Real fdT=1e-2;
  Real fdmu=1e-2;
  const char *my_init_str="";
  const char *kspace_labels="";
  ...
  Real active2full=(Real)(lattice_only.atom_pos.get_size())/(Real)(lattice.atom_pos.get_size());
  ...
  if (!rnd_walk) { // do standard grid scan;
	...
	pmc->init_structure(*my_p_init_str);
      ...
      Array<Real> full_mu,full_mu_af;
      extract_elements(&full_mu,cur_control,1,cur_control.get_size());
      product(&full_mu_af,full_mu,active2full); //fix
      Array<Real> mu;
      product(&mu,full_mu_af,corr_to_fullconc);
      Real muxc=inner_product(full_mu_af,conc_to_fullconc_c);
	...
	product(&ptcorr_lte,ptcorr_lte,active2full); //fix
	...
	if (addmux) {
	  Real mux=inner_product(full_mu_af,x_lte);
	...
	product(&ptcorr_mf,ptcorr_mf,active2full); //fix
	...
	if (addmux) {
	  Real mux=inner_product(full_mu_af,x_mf);
	...
	if (pdot) {
	  ...
	  *pdot='.';
      ...
      product(&ptcorr_mc,ptcorr_mc,active2full); //fix
	...
	for (int ll=looplevel; ll>=0; ll--) {phi(ll)=new_phi;}
      ...
      if (addmux) {
	Real mux=inner_product(full_mu_af,x_mc);
	...
	line << endl << '\0';
	mcfile << line.str().c_str() << flush;
	if (!quiet) {cout << line.str().c_str() << flush;}
      }
      if (quitloop) {
	...
	print_vector(line,corr,delim);
	line << endl << '\0';
	if (!reject) {mcfile << line.str().c_str() << flush;}
	if (!quiet) {cout << (reject ? 'R' : 'A') << delim << line.str().c_str() << flush;}
      }
      if (reject) {
