...
void calc_convex_hull(Array<int> *p_gs_str, const Array<Real> &concentration,
                      const Array<Real> &energy) {
  ...
  for (int i = 0; i < energy.get_size(); i++) {
    if ((concentration(i) < cur_concentration) ||
        (concentration(i) == cur_concentration && energy(i) < cur_energy)) {
  ...
  while (1) {
    ...
    for (int i = 0; i < energy.get_size(); i++) {
      if (concentration(i) > cur_concentration) {
        Real slope =
            (energy(i) - cur_energy) / (concentration(i) - cur_concentration);
        if (slope < best_slope) {
          best_slope = slope;
          cur_gs = i;
        }
...
void convex_hull_from_gs(Array<Real> *p_gs_concentration,
                         Array<Real> *p_gs_energy, const Array<int> &gs_str,
                         const Array<Real> &concentration,
                         const Array<Real> &energy) {
  p_gs_concentration->resize(gs_str.get_size());
  p_gs_energy->resize(gs_str.get_size());
...
int ground_state_problem(Array<int> *p_problem, const Array<int> &ground_state,
                         const Array<Real> &concentration,
                         const Array<Real> &fitted_energy, Real minc,
                         Real maxc) {
  ...
  if (near_zero(maxc - minc)) {
    ...
    for (int str = 0; str < concentration.get_size(); str++) {
      if (near_zero(concentration(str) - minc) &&
          fitted_energy(str) < min_energy) {
    ...
    if (index_min != -1) {
      ...
      for (gs = 0; gs < ground_state.get_size(); gs++) {
        if (concentration(ground_state(gs)) > minc - zero_tolerance)
          break;
      }
    ...
    for (min_gs = 1; min_gs < ground_state.get_size() - 1; min_gs++) {
      if (concentration(ground_state(min_gs)) - zero_tolerance > minc)
        break;
    }
    int max_gs;
    for (max_gs = ground_state.get_size() - 1; max_gs > 1; max_gs--) {
      if (concentration(ground_state(max_gs - 1)) + zero_tolerance < maxc)
        break;
    }
    ...
    for (int gs = min_gs; gs <= max_gs; gs++) {
      ...
      for (int str = 0; str < fitted_energy.get_size(); str++) {
        ...
        if (c >= c0 && c <= c1) {
          Real hull_e = a + b * c;
          if (str != ground_state(gs - 1) && str != ground_state(gs) &&
              fitted_energy(str) <= hull_e) {
            (*p_problem)(str) = 1;
            (*p_problem)(ground_state(gs - 1)) = 1;
            (*p_problem)(ground_state(gs)) = 1;
            a_problem = 1;
...
int ground_state_problem(Array<int> *p_problem,
                         const Array<Real> &gs_concentration,
                         const Array<Real> &gs_energy,
                         const Array<Real> &concentration,
                         const Array<Real> &energy, Real minc, Real maxc) {
  ...
  for (min_gs = 1; min_gs < gs_concentration.get_size() - 1; min_gs++) {
    if (gs_concentration(min_gs) - zero_tolerance > minc)
      break;
  }
  int max_gs;
  for (max_gs = gs_concentration.get_size() - 1; max_gs > 1; max_gs--) {
    if (gs_concentration(max_gs - 1) + zero_tolerance < maxc)
      break;
  }
  for (int gs = min_gs; gs <= max_gs; gs++) {
...
void calc_formation(Array<Real> *p_form_value, const Array<Real> &concentration,
                    const Array<Real> &value, Real *e_c0, Real *e_c1) {
  ...
  if (e_c0 && e_c1 && (*e_c0) != MAXFLOAT && (*e_c1) != MAXFLOAT) {
    ...
    for (int i = 0; i < value.get_size(); i++) {
      if (concentration(i) == minc)
        mince = value(i);
      if (concentration(i) == maxc)
        maxce = value(i);
    }
    if (near_zero(maxc - minc)) {
      pure_A = mince;
...
void calc_hull_energy(Array<Real> *hull_energy, const Array<int> &ground_state,
                      const Array<Real> &concentration,
                      const Array<Real> &energy) {
