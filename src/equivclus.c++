...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (!clusfile)
      ERRORQUIT("Unable to read cluster.");
  ...
  */
  ...
  for (op = 0; op < spacegroup.point_op.get_size(); op++) {
    ...
    apply_symmetry(&transpos0, spacegroup.point_op(op), spacegroup.trans(op),
                   cluspos(0));
    rVector3d shift;
    if (equivalent_mod_cell(cluspos(1), transpos0, invcell, &shift)) {
      ...
      for (at = 0; at < transpos0.get_size(); at++) {
        // cerr << "--" << endl;
        // cerr << cluslabel(1)(which_atom(cluspos(1),transpos0(at)-shift)) <<
        // endl; cerr << cluslabel(0)(at) << endl;
        if (strcmp(cluslabel(1)(which_atom(cluspos(1), transpos0(at) - shift)),
                   cluslabel(0)(at)) != 0)
          break;
      }
      if (at == transpos0.get_size())
        break;
    }
  }
