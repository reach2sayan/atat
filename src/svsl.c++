...
template <> GenericPlugIn<ArrayFunctionArray<Real> > *GenericPlugIn<ArrayFunctionArray<Real> >::list=NULL;

int main(int argc, char *argv[]) {
  const char *latfilename="";
  const char *strfilename="str.out";
  const char *rel_strfilename="str_relax.out";
  const char *sprfilename="slspring.out";
  const char *potplugin="";
  const char *massfilename="";
  Real bulkmod=0.;
  const char *bulkmodfile="";
  Real maxs=0.05;
  int ns=1;
  Real T0=0.;
  Real T1=2000.;
