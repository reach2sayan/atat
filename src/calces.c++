...
int main(int argc, char *argv[]) {
  ...
  if (!file)
    ERRORQUIT("Unable to open unrelaxed file.");
  ...
  while (1) {
    ...
    for (int i = 0; i < ustr.atom_pos.get_size(); i++) {
      int j = index_in_array(species_name,
                             atom_label(site_type_list(ustr.atom_type(i))(0)));
      if (j == -1) {
        cerr << "Unable to find specie"
             << atom_label(site_type_list(ustr.atom_type(i))(0)) << endl;
        ERRORQUIT("Aborting");
      }
