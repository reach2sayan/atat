...
void find_equivalent_clusters(Array<ArrayrVector3d> *pclusters,
  ...
  for (int op = 0; op < point_op.get_size(); op++) {
    Array<rVector3d> *ptransfo_cluster =
        new Array<rVector3d>(cluster.get_size());
    apply_symmetry(ptransfo_cluster, point_op(op), trans(op), cluster);
    LinkedListIterator<ArrayrVector3d> i(list);
    for (; i; i++) {
      if (equivalent_mod_cell(*i, *ptransfo_cluster, inv_cell))
        break;
    }
    if (!i) {
...
int calc_multiplicity(const Array<rVector3d> &cluster, const rMatrix3d &cell,
                      const Array<rMatrix3d> &point_op,
                      const Array<rVector3d> &trans) {
...
Real calc_correlation(const Structure &str,
                      const Array<ArrayrVector3d> &clusters,
                      const rMatrix3d &cell) {
  Real accum = 0.;
  int count = 0;
  ...
  for (; t; t++) {
    for (int c = 0; c < clusters.get_size(); c++) {
      ...
      for (int at = 0; at < clusters(c).get_size(); at++) {
        int spin = str.atom_type(
            which_atom(str.atom_pos, t + clusters(c)(at), inv_strcell));
...
Real calc_correlation(const Structure &str, const Array<rVector3d> &cluster,
                      const rMatrix3d &cell, const Array<rMatrix3d> &point_op,
                      const Array<rVector3d> &trans) {
...
void find_clusters_overlapping_site(
    Array<ArrayrVector3d> *o_cluster, Array<Real> *o_eci, const rVector3d &site,
    const Structure &lattice, const SpaceGroup &space_group,
    const LinkedList<ArrayrVector3d> &cluster_list,
    const LinkedList<Real> &eci_list, int skipzeros) {
  ...
  for (; c; c++, e++) {
    if (c->get_size() > 0) {
      if (!(skipzeros && (*e) == 0)) {
        Array<ArrayrVector3d> clusters;
        find_equivalent_clusters(&clusters, *c, space_group.cell,
                                 space_group.point_op, space_group.trans);
        for (int ec = 0; ec < clusters.get_size(); ec++) {
          for (int center_atom = 0; center_atom < clusters(ec).get_size();
               center_atom++) {
            rVector3d lat_shift = site - clusters(ec)(center_atom);
            if (is_int(inv_cell * lat_shift)) {
              ArrayrVector3d *to_add =
                  new ArrayrVector3d(clusters(ec).get_size());
              int j = 0;
              (*to_add)(j) = site;
              j++;
...
/*
void calc_correlation(Array<Real> *correlation, int components, const Structure
&str, const Array<ArrayrVector3d> &clusters, const rMatrix3d &cell) { Real
accum=0.; int count=0; LatticePointInCellIterator t(cell,str.cell); for ( ;t;
t++) { for (int c=0; c<clusters.get_size(); c++) { Real sigma=1.; for (int at=0;
at<clusters(c).get_size(); at++) {
...
Real find_empty_eci(const LinkedList<ArrayrVector3d> &cluster_list,
...
void CorrFuncTable::init_from_site_type_list(
    const Array<Array<int>> &site_type_list) {
  int c = 0;
...
void find_equivalent_clusters(Array<MultiCluster> *pclusters,
                              const MultiCluster &cluster,
                              const rMatrix3d &cell,
                              const Array<rMatrix3d> &point_op,
                              const Array<rVector3d> &trans) {
  rMatrix3d inv_cell = !cell;
...
int calc_multiplicity(const MultiCluster &cluster, const rMatrix3d &cell,
                      const Array<rMatrix3d> &point_op,
                      const Array<rVector3d> &trans) {
...
Real calc_correlation(const Structure &str, const Array<MultiCluster> &clusters,
                      const rMatrix3d &cell,
  ...
  for (; t; t++) {
    for (int c = 0; c < clusters.get_size(); c++) {
      ...
      for (int at = 0; at < clusters(c).clus.get_size(); at++) {
        Real spin = corrfunc(clusters(c).site_type(at))(
            clusters(c).func(at))(str.atom_type(
            which_atom(str.atom_pos, t + clusters(c).clus(at), inv_strcell)));
        sigma = sigma * spin;
      }
      accum += sigma;
      count++;
...
Real calc_correlation_fast(Tensor<int> *ptab_str, const Structure &str,
                           const Array<MultiCluster> &clusters,
                           const Structure &lat,
                           const Array<Array<Array<Real>>> &corrfunc) {
  ...
  if (ptab_str->get_size().get_size() == 0) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        if (i != j) {
          if (!near_zero(supscale(i, j))) {
            ERRORQUIT("Fast algorithm needs a simple supercell");
          }
        } else {
          supcel(i) = iround(supscale(i, j));
        }
      }
    }
    ...
    for (int at = 0; at < str.atom_pos.get_size(); at++) {
      ...
      if (atl == -1) {
        ERRORQUIT("Lattice/structure mismatch");
      }
      iVector3d t = to_int(inv_cell * (str.atom_pos(at) - lat.atom_pos(atl)));
      Array<int> cellat(4);
      for (int i = 0; i < 3; i++) {
        cellat(i) = neg_mod(t(i), supcel(i));
      }
      cellat(3) = atl;
  ...
  for (int c = 0; c < clusters.get_size(); c++) {
    ...
    for (int p = 0; p < clusters(c).clus.get_size(); p++) {
      ...
      if (atl == -1) {
        ERRORQUIT("Lattice/cluster mismatch");
      }
      iVector3d t =
          to_int(inv_cell * (clusters(c).clus(p) - lat.atom_pos(atl)));
  ...
  for (; cell; cell++) {
    for (int c = 0; c < clusters.get_size(); c++) {
      ...
      for (int at = 0; at < clusters(c).clus.get_size(); at++) {
        ...
        for (int i = 0; i < 3; i++) {
          cellat(i) =
              neg_mod(((iVector3d &)cell)(i) + clus_int(c)(at)(i), supcel(i));
        }
        cellat(3) = clus_int(c)(at)(3);
        Real spin = corrfunc(clusters(c).site_type(at))(clusters(c).func(at))(
            (*ptab_str)(cellat));
        sigma = sigma * spin;
      }
      accum += sigma;
...
void find_clusters_overlapping_site(
    Array<MultiCluster> *o_cluster, Array<int> *o_which, const rVector3d &site,
  ...
  for (; c; c++, e++) {
    if (c->clus.get_size() > 0) {
      ...
      find_equivalent_clusters(&clusters, *c, space_group.cell,
                               space_group.point_op, space_group.trans);
      for (int ec = 0; ec < clusters.get_size(); ec++) {
        for (int center_atom = 0; center_atom < clusters(ec).clus.get_size();
             center_atom++) {
          rVector3d lat_shift = site - clusters(ec).clus(center_atom);
          if (is_int(inv_cell * lat_shift)) {
            MultiCluster *to_add =
                new MultiCluster(clusters(ec).clus.get_size());
            int j = 0;
            to_add->clus(j) = site;
...
void calc_corr_to_conc(Array2d<Real> *pmat, const Structure &lat,
                       const Array<Array<int>> &site_type_list,
                       const SpaceGroup &spacegroup,
                       const Array<Array<Array<Real>>> &corrfunc,
                       Array<Real> *p_pt_mult) {
  int total_species = 0;
  for (int i = 0; i < site_type_list.get_size(); i++) {
  ...
  */
  ...
  for (int i = 0; i < nb_species.get_size(); i++, c++) {
    nb_species(i) =
        site_type_list(
            lat.atom_type(which_atom(lat.atom_pos, c->clus(0), invcell)))
            .get_size();
    Array<MultiCluster> eq_clusters;
    find_equivalent_clusters(&eq_clusters, *c, lat.cell, spacegroup.point_op,
                             spacegroup.trans);
    equiv_site(i).resize(eq_clusters.get_size());
    for (int j = 0; j < eq_clusters.get_size(); j++) {
      equiv_site(i)(j) =
          which_atom(lat.atom_pos, eq_clusters(j).clus(0), invcell);
    }
  }

  Array<int> endpt(nb_species);
  ...
  if (p_pt_mult) {
    ...
    for (; c; c++, i++) {
      (*p_pt_mult)(i) =
          calc_multiplicity(*c, spacegroup.cell, spacegroup.point_op,
                            spacegroup.trans) /
          (Real)(lat.atom_type.get_size());
    }
  }

  int nb_in_basis = 0;
  for (int site = 0; site < endpt.get_size(); site++) {
    zero_array(&endpt);
    for (endpt(site) = 0; endpt(site) < nb_species(site); endpt(site)++) {
      ...
      for (; c; c++) {
        Real corr =
            calc_correlation(str, *c, spacegroup.cell, spacegroup.point_op,
                             spacegroup.trans, corrfunc);
        if (p_pt_mult) {
          corr *= calc_multiplicity(*c, spacegroup.cell, spacegroup.point_op,
                                    spacegroup.trans) /
                  (Real)(lat.atom_type.get_size());
        }
...
void extract_nonredundant(Array2d<Real> *pnored, Array2d<Real> *pnoredtored,
                          Array<Real> *pconst, const Array2d<Real> &mat) {
  ...
  Array2d<Real> bignoredt, mat_bignoredt, bignored_bignoredt, inv,
      bignoredtored;
...
void calc_concentration(Array<Real> *pconc, const Array<int> &atom_type,
                        int nbatomtype) {
...
int calc_concentration(Array<Real> *pconc, const Structure &lattice,
                       const Array<Array<int>> &site_type_list,
                       const Structure &str) {
  int comp = 0;
  for (int i = 0; i < site_type_list.get_size(); i++) {
...
void calc_noredtored(Array2d<Real> *pmat, Array<Real> *pvec,
                     const Structure &lat,
                     const Array<Array<int>> &site_type_list) {
  int total_species = 0;
  for (int i = 0; i < site_type_list.get_size(); i++) {
...
void calc_redtonored(Array2d<Real> *pmat, const Array2d<Real> &noredtored,
                     const Array<Real> &noredtored_c, int keepconst) {
...
void find_clusters_symmetry(SpaceGroup *psubgroup, const MultiCluster &cluster,
                            const SpaceGroup &space_group) {
  rMatrix3d inv_cell = !(space_group.cell);
  ...
  for (int op = 0; op < space_group.point_op.get_size(); op++) {
    ...
    apply_symmetry(&transfo_cluster, space_group.point_op(op),
                   space_group.trans(op), cluster);
...
Real calc_correlation_atomcluster(const Array<rVector3d> &atom_pos,
    ...
    for (int p = 0; p < atom_pos.get_size(); p++) {
      ...
      if (is_int(icell * shift)) {
        ...
        for (i = 0; i < clusters(c).clus.get_size(); i++) {
          ...
          Real spin = corrfunc(clusters(c).site_type(i))(clusters(c).func(i))(
              atom_type(at));
          sigma *= spin;
        }
