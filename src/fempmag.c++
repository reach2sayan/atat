...
Real calc_empirical_mag_free_energy(Real spin, Real T, Real Tc, Real kB,
				    Real p) {
  Real tau = T / Tc;
  Real A = 518. / 1125. + (11692. / 15975.) * ((1. / p) - 1.);
  Real tauf;
  if (tau < 1.) {
    tauf = tau - (1. / A) * (79. / (140. * p) +
			     tau * (474. / 497.) * ((1. / p) - 1) *
...
int main(int argc, char *argv[]) {
  ...
  AskStruct options[] = {
      {"",
       "Empirical magnetic free energy calculator, version " MAPS_VERSION
       ", by Axel van de Walle",
       TITLEVAL, NULL},
      {"-s", "Input file defining the structure (default: str.out)", STRINGVAL,
       &strfilename},
      {"-tc", "Input file containing the Currie Temperature (default: tcurrie)",
       STRINGVAL, &Tcurriefilename},
      {"-mm",
       "Input file containing the TOTAL magnetic moment (default: magmom)",
       STRINGVAL, &magmomfilename},
      {"-p",
       "Ratio of magnetic enthalpy due to SRO to total enthalpy (for fcc or "
       "hcp, use 0.28 (default), for bcc, use 0.4)",
       REALVAL, &p},
      ...
      {"-sc", "Correction factor if spectator ion are present", REALVAL,
       &perfewer},
  ...
  Real Tcurrie = 0.;
  {
    ...
    if (!file) {
      ERRORQUIT("Unable to open currie temperature file");
    }
    file >> Tcurrie;
  }
  Real magmom = 0.;
  Real denom = (Real)(str.atom_pos.get_size());
  {
    ...
    if (!file) {
      ERRORQUIT("Unable to open magnetic moment file");
    }
    file >> magmom;
  ...
  }
  {
    ...
    for (Real T = T0; T <= T1 + zero_tolerance; T += dT) {
      Real F =
	  calc_empirical_mag_free_energy(magmom, T, Tcurrie, kboltzman, p) *
	  (peratom ? 1. : denom) * perfewer;
