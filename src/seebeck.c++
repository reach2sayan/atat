...
class KBand {
 public:
  KBand(void) : k(), band(), v() {}
  KBand(const rVector3d &_k, const Array<Real> &_band)
      : k(_k), band(_band), v() {}
  rVector3d k;
  Array<Real> band;
  Array<rVector3d> v;
...
int main(int argc, char *argv[]) {
  ...
  AskStruct options[] = {
      {"",
       "Seebeck coefficient calculator" MAPS_VERSION ", by Axel van de Walle",
       TITLEVAL, NULL},
      //    {"-h","Display more help",BOOLVAL,&dohelp},
      {"-s", "Input file defining the structure (default: str.out)", STRINGVAL,
       &strfilename},
      {"-bf", "Input file defining the band structure (default: bands.out)",
       STRINGVAL, &bandfilename},
  ...
  while (1) {
	...
	for (; i; i++) {
	  if (cylinder_norm(inv_rec_lat * (Eonek2.k - (i->k))) < zero_tolerance)
	    break;
	}
	if (!i) {
  ...
  for (; i; i++) {
	...
	Real cv = calc_cv(x, y);
	cerr << "band= " << b << " n= " << nb_nei << " R2= " << R2
	     << " cv= " << cv
	     << " mse= " << sqrt(nsigma2 / (Real)(y.get_size())) << endl;
      }
      for (int d = 0; d < 3; d++) {
	i->v(b)(d) = der(d);
      }
  ...
  for (int n = 0; n < nE; n++) {
    logfile << Emin + dE * (Real)n << " " << dos(n) << " " << trsigma(n)
	    << endl;
  }
}
