...
int main(int argc, char *argv[]) {
  ...
  AskStruct options[] = {
      {"",
       "STRAIN DERivative calculator " MAPS_VERSION ", by Axel van de Walle",
       TITLEVAL, NULL},
      {"-f", "Fit (instead of the default, generate perturbations)", BOOLVAL,
       &fit},
      {"-s",
      ...
      {"-si", "Symmetric indexes (comma-separated list; indices start at 0)",
       STRINGVAL, &sym_indices},
  ...
  if (!fit) {
    ...
    if (nozero) {
      ...
      if (!tensorfile) {
	cerr << " Unable to open file " << tensorfilename << " for zero strain"
	     << endl;
	ERRORQUIT("Aborting");
      }
    ...
    Array2d<Real> big_eqnmat(tensor_list.get_size() * block,
			     tensor_basis.get_size());
	...
	for (int j = 0; i; i++, j++) {
	  big_eqnmat(p * block + (j % block), b) +=
	      tensor_basis(b)(ai) * (*istrain)(ai(rank + 0), ai(rank + 1));
	}
      }
    }
    Array<Real> beta;
	...
	logfile << big_tensor(i) << " " << tensor_hat(i) << " "
		<< (big_tensor(i) - tensor_hat(i)) << endl;
      }
      logfile << "end" << endl;
    }
    ...
    if (proper) {
      if (rank != 1) {
	ERRORQUIT("rank must be 1 for this option.");
      }
      MultiDimIterator<Array<int> > i(resptensorsize);
      Array<int> &ai = (Array<int> &)i;
      for (; i; i++) {
	strainder(i) += (ai(1) == ai(2) ? 1. : 0.) * atzero.vectorize()(ai(0)) -
			(ai(0) == ai(1) ? 1. : 0.) * atzero.vectorize()(ai(2));
      }
    }
