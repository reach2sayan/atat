...
int main(int argc, char *argv[]) {
  ...
  AskStruct options[] = {
      {"",
       "Free energy - MAGnetic contribution " MAPS_VERSION
  ...
  for (int at = 0; at < labellookup.get_size(); at++) {
    if (labellookup(at).get_size() != 1) {
      ERRORQUIT("Not a structure file. Aborting.");
    }
  ...
  for (; p.length() < rnn; p++) {
    if (str.atom_type(which_atom(str.atom_pos, p(0), invcell)) == magindex &&
	str.atom_type(which_atom(str.atom_pos, p(1), invcell)) == magindex) {
      nbmagpair += 1.;
    }
    nbpair += 1.;
  }
  ...
  for (Real T = T0; T <= T1 + zero_tolerance; T += dT) {
    Real f =
	nbmag * calc_mag_free_energy(maxspin, coupling * (nbmagpair / nbpair),
				     kboltzman * T);
    Real s = nbmag * find_avg_spin(maxspin, coupling * (nbmagpair / nbpair) /
						(kboltzman * T));
    cout << T << '\t' << f << '\t' << s << endl;
  }
}
