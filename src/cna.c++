...
public:
  ...
  AtomAndType(const AtomAndType &a)
      : pos(a.pos), type(a.type), index(a.index) {}
  AtomAndType(const rVector3d &_pos, int _type, int _index)
      : pos(_pos), type(_type), index(_index) {}
};
...
void common_neighbor_analysis(LinkedList<Array<int>> *pall_common_neighbor,
                              const Structure &str, const Array<Real> &cutoff,
                              int shell, int cshell) {
  ...
  for (int i = 0; i < str.atom_pos.get_size(); i++) {
    ...
    for (; rj; rj++) {
      if (!done(rj->index)) {
        ...
        common_neighbor << new AtomAndType(str.atom_pos(i), str.atom_type(i),
                                           i);
        // cerr << str.atom_pos(i) << endl;
        ...
        cn_code(k + 1) = rj->type;
        if (cn_code(k) < cn_code(k + 1)) {
          swap(&(cn_code(k)), &(cn_code(k + 1)));
        }
        LinkedListIterator<Array<int>> icn(*pall_common_neighbor);
        for (; icn; icn++) {
          if (array_order(*icn, cn_code, 1) != -1)
            break;
        }
        if (!icn) {
...
int main(int argc, char *argv[]) {
  ...
  int cshell = 1;
  AskStruct options[] = {
      {"", "Common Neighbor Analysis code, by Axel van de Walle", TITLEVAL,
       NULL},
      {"",
       "  First run with -r option, copy shells.out to shells.in and then run "
       "with -s",
       TITLEVAL, NULL},
      {"-r", "Maximum radius for calculating pair correlations", REALVAL,
       &rmax},
  ...
  if (shell < cshell)
    ERRORQUIT("Error: must have s>=cs");
  if (rmax > 0.) {
    ...
    while (!infile.eof()) {
      ...
      parse_lattice_file(&(str.cell), &(str.atom_pos), &(str.atom_type),
                         &site_type_list, &atom_label, infile);
    ...
    Array<Real> cutoff;
    {
      ...
      if (!file)
        ERRORQUIT("Unable to open shells.in");
      LinkedList<Real> cutofflist;
      while (!file.eof()) {
        Real c;
        file >> c;
    ...
    while (!infile.eof()) {
      ...
      wrap_inside_cell(&(str.atom_pos), str.atom_pos, str.cell);
      fix_atom_type(&str, site_type_list);
      common_neighbor_analysis(&all_common_neighbor, str, cutoff, shell,
                               cshell);
      skip_to_next_structure(infile);
    }
    Real total = 0;
