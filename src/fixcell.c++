...
int main(int argc, char *argv[]) {
  ...
  AskStruct options[] = {
      ...
      {"-b",
       "print bravais lattice type only and conventional (potentially not "
       "primitive) cell",
       BOOLVAL, &bravaistype},
  ...
  if (dohelp) {
    cout << "Reads a cell from stdin writes a 'symmetrized' version of it to "
            "stdout"
         << endl;
    return 1;
  }
  ...
  if (fabs(det(cell)) < zero_tolerance)
