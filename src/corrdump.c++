...
int main(int argc, char *argv[]) {
  ...
  int printnum = 0;
  AskStruct options[] = {
      {"", "CORRelation DUMPer " MAPS_VERSION ", by Axel van de Walle",
       TITLEVAL, NULL},
      ...
      {"-3", "Maximum distance between two points within a triplet", REALVAL,
       &maxd(3)},
      {"-4", "Maximum distance between two points within a quadruplet", REALVAL,
       &maxd(4)},
      {"-5", "Maximum distance between two points within a quintuplet", REALVAL,
       &maxd(5)},
      ...
      {"-fast",
       "Use fast algo to calculate correlations of 'simple' supercells",
       BOOLVAL, &fastalgo},
      ...
      {"-cf", "File name of the cluster file (default: clusters.out)",
      ...
      {"-wu", "Write Unrelaxed structure into specified file", STRINGVAL,
       &writeunrel},
      {"-rnd",
       "Print correlation of the random state of the same composition as the "
       "input structure",
       BOOLVAL, &rndcorr},
      {"-eci", "Predict quantity using ECI in specified file", STRINGVAL,
       &ecifile},
      {"-mi", "Multiplicities are already included in ECI file", BOOLVAL,
       &multincl},
      //    {"-dil","Input file defining the dilute
      //    sites",STRINGVAL,&dilutefilename},
      {"-crf", "Select correlation functions (default: trigo)", STRINGVAL,
       &corrfunc_label},
      {"-nb", "Print structure number", BOOLVAL, &printnum},
      {"-ro",
       "Read lattice file containing occupation variables (this code does not "
       "make use of them)",
       BOOLVAL, &readocc}};
  ...
  rMatrix3d axes;
  {
    ...
    if (readocc) {
      ...
      parse_rndstr_file(&lattice.cell, &lattice.atom_pos, &lattice.atom_type,
                        &occ, &labellookup, &label, latticefile, &axes);
    } else {
  ...
  find_spacegroup(&spacegroup.point_op, &spacegroup.trans, lattice.cell,
                  lattice.atom_pos, lattice.atom_type);
  ...
  for (nbsite = 0; nbsite < lattice.atom_pos.get_size(); nbsite++) {
    if (labellookup(lattice.atom_type(nbsite)).get_size() == 1)
      break;
  }
  if (nbsite == 0 && strlen(dilutefilename) == 0)
    ERRORQUIT("Need at least one site with multiple species in lattice file.");
  ...
  if (strlen(dilutefilename) > 0) {
    ...
    if (!dilfile)
      ERRORQUIT("Unable to open dilute sites file.");
    while (!dilfile.eof()) {
      AutoString tmp;
  ...
  if ((!doconc && !doconcmat) || strlen(ecifile) > 0) {
    if (!readclusters && !noclus) {
      ...
      while (maxd(maxp) == 0 && maxp > 0)
        maxp--;
      if (maxp == 0)
        ERRORQUIT("Specify -2=range -3=range etc.");
      ...
      for (int m = 2; m <= maxp; m++) {
        ...
        if (maxd(m) > 0) {
          while (get_length_quick((*pclusterbanks(m))->clus) <
                 maxd(m) + zero_tolerance)
            (*pclusterbanks(m))++;
        }
        nbcluster(m) = pclusterbanks(m)->get_current_index();
      }
      if (!noempty)
        clusterlist << new MultiCluster(0);
        ...
        for (; icluster; icluster++) {
          ...
          for (int i = 0; i < icluster->clus.get_size(); i++) {
            clusterfile << ((!axes) * (icluster->clus(i))) << " "
                        << icluster->site_type(i) << " " << icluster->func(i)
                        << endl;
          }
          clusterfile << endl;
        }
      ...
      if (!clusterfile)
        ERRORQUIT("Unable to open cluster file");
  ...
  if (doconcmat) {
    Array2d<Real>
        corr_to_fullconc; // converts correlation to redundant concentration;
    Array2d<Real>
        corr_to_conc; // converts correlation to non redundant concentration;
    Array2d<Real> conc_to_fullconc; // converts nonredundant concentration to
                                    // full redundant concentration;
    Array<Real> conc_to_fullconc_c; // constant terms of previous matrix;
    calc_corr_to_conc(&corr_to_fullconc, lattice, labellookup, spacegroup,
                      *pcorrfunc);
    extract_nonredundant(&corr_to_conc, &conc_to_fullconc, &conc_to_fullconc_c,
                         corr_to_fullconc);
    cout.setf(ios::fixed);
  ...
  if (strlen(ecifile) > 0) {
    ...
    if (!file)
      ERRORQUIT("Unable to open ECI file.");
    ...
    if (clusterlist.get_size() != eci.get_size())
  ...
  if (doclusatom) {
    ...
    if (strlen(ecifile) == 0) {
      ERRORQUIT("Please specify -eci=[ecifile].");
    }
    Cluster lastclus;
    LinkedListIterator<MultiCluster> icluster(clusterlist);
    for (; icluster; icluster++) {
      if (icluster->clus.get_size() > 0 &&
          !spacegroup(icluster->clus, lastclus)) {
        ...
        for (; occ; occ++) {
          ...
          for (int ieci = 0; ipcluster; ieci++, ipcluster++) {
            Real corr = calc_correlation_atomcluster(
                icluster->clus, occ, *ipcluster, spacegroup.cell, *pcorrfunc);
            if ((*ipcluster)(0).clus.get_size() == 0) {
              corr *= (Real)(icluster->clus.get_size()) /
                      (Real)(lattice_only.atom_pos.get_size());
            }
            energy += corr * eci(ieci);
          }
          // cout << icluster->clus << endl << (Array<int> &)occ << endl <<
          // energy << endl;
          ...
          for (int i = 0; i < aocc.get_size(); i++) {
            ...
            cout << label(labellookup(which_atom(
                lattice_only.atom_pos, icluster->clus(i), icell))(aocc(i)));
          }
          cout << " " << get_length_quick(icluster->clus) << " " << energy
               << endl;
        }
      }
  ...
  if (!strfile)
    ERRORQUIT("Unable to open structure file");
  int strnum = 1;
  while (!strfile.eof()) {
    ...
    if (skiprel) {
      ...
      find_all_atom_in_supercell(&ideal_str.atom_pos, &ideal_str.atom_type,
                                 lattice_only.atom_pos, lattice_only.atom_type,
                                 lattice_only.cell, ideal_str.cell);
      ...
      for (int i = 0; i < str.atom_pos.get_size(); i++) {
        ...
        for (int j = 0; j < str.atom_pos.get_size(); j++) {
          for (int k = 0; k < ideal_str.atom_pos.get_size(); k++) {
            if (str.atom_type(j) != -1 && ideal_str.atom_type(k) != -1) {
              if (is_in_array(labellookup(ideal_str.atom_type(k)),
                              str.atom_type(j))) {
        ...
        if (min_d != MAXFLOAT) {
          logfile << label(str.atom_type(best_j)) << " atom at "
                  << ((!axes) * (str.atom_pos(best_j)))
                  << " mapped onto lattice site at "
                  << ((!axes) * (ideal_str.atom_pos(best_k)))
                  << " distance= " << min_d << endl;
          spin(best_k) = index_in_array(
              labellookup(ideal_str.atom_type(best_k)), str.atom_type(best_j));
      ...
      for (vac = 0; vac < label.get_size(); vac++) {
        if (strcmp(label(vac), "Vac") == 0)
          break;
        if (strcmp(label(vac), "Va") == 0)
          break;
      }
      for (int k = 0; k < ideal_str.atom_pos.get_size(); k++) {
        if (ideal_str.atom_type(k) != -1) {
          if (!is_in_array(labellookup(ideal_str.atom_type(k)), vac))
            ERRORQUIT("Unexpected vacancy");
          spin(k) = index_in_array(labellookup(ideal_str.atom_type(k)), vac);
          logfile << "Vacancy at " << ((!axes) * (ideal_str.atom_pos(k)))
                  << endl;
        }
      }
      ...
      for (int i = 0; i < str.atom_type.get_size(); i++) {
        if (str.atom_type(i) != -1) {
          logfile << label(str.atom_type(i)) << " atom ignored at "
                  << ((!axes) * (str.atom_pos(i))) << endl;
        }
      }
    }
    if (strlen(writeunrel) > 0) {
      ...
      if (!file) {
        ERRORQUIT("Unable to write unrelaxed structure");
      }
      Structure full_ideal_str;
      full_ideal_str.cell = ideal_str.cell;
      find_all_atom_in_supercell(
          &full_ideal_str.atom_pos, &full_ideal_str.atom_type, lattice.atom_pos,
          lattice.atom_type, lattice.cell, full_ideal_str.cell);
      int at0 = 0;
      for (int at = 0; at < full_ideal_str.atom_type.get_size(); at++) {
        if (labellookup(full_ideal_str.atom_type(at)).get_size() > 1) {
          if (norm(full_ideal_str.atom_pos(at) - ideal_str.atom_pos(at0)) >
              zero_tolerance) {
            ERRORQUIT("Position mismatch in write unrelaxed feature");
          }
    ...
    if (rndcorr) {
      if (nopoint)
        ERRORQUIT("Please do NOT specify -nop with -rnd.");
      ...
      for (; ipcluster; ipcluster++) {
        if ((*ipcluster)(0).clus.get_size() > 1)
          break;
        if ((*ipcluster)(0).clus.get_size() == 1) {
          pointcorr << new Real(calc_correlation(ideal_str, *ipcluster,
                                                 spacegroup.cell, *pcorrfunc));
        }
      }
      int ieci = 0;
      LinkedListIterator<MultiCluster> icluster(clusterlist);
      ...
      for (; icluster; icluster++, ieqcluster++, ieci++) {
        ...
        for (int s = 0; s < icluster->clus.get_size(); s++) {
          ...
          while (ipcluster->clus.get_size() == 0) {
            ipcluster++;
          }
          ...
          while (1) {
            if (ipcluster->site_type(0) == icluster->site_type(s) &&
              ...
              if (equivalent_by_symmetry(ipcluster->clus(0), icluster->clus(s),
      ...
      for (; icluster; icluster++, ieci++) {
        Real rho;
        if (fastalgo) {
          rho = calc_correlation_fast(&tab_str, ideal_str, *icluster, lattice,
                                      *pcorrfunc);
        } else {
          rho = calc_correlation(ideal_str, *icluster, spacegroup.cell,
                                 *pcorrfunc);
        }
        if (strlen(ecifile) > 0) {
