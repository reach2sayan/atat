...
int main(int argc, char *argv[]) {
  ...
  if (!quiet)
    cout << "Enter original cell: (in cartesian, one lattice vector per line "
            "with coordinates separated by spaces)"
         << endl;
  read_cell(&before, cin);
  if (!quiet)
    cout << "Enter relaxed cell: (in cartesian, one lattice vector per line "
            "with coordinates seperated by spaces)"
         << endl;
  read_cell(&after, cin);
  rMatrix3d Id;
  Id.identity();
