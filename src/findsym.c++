...
int equivalent_by_symmetry(const rVector3d &a, const rVector3d &b,
  ...
  for (int op = 0; op < point_op.get_size(); op++) {
    ...
    if (equivalent_mod_cell(a, t_b, inv_cell))
      return 1;
  }
  return 0;
}
...
int equivalent_by_symmetry(const Structure &a, const Structure &b,
                           ...
                           const Array<rMatrix3d> &point_op,
                           const Array<rVector3d> &trans) {
  if (a.atom_pos.get_size() > b.atom_pos.get_size())
    return equivalent_by_symmetry(b, a, cell, point_op, trans);
  if ((b.atom_pos.get_size() % a.atom_pos.get_size()) != 0)
    return 0;
  //  if
  //  (!near_zero(check_sum_structure(a.atom_type)-check_sum_structure(b.atom_type)))
  //  return 0;
  ...
  if (min_typea != min_typeb || max_typea != max_typeb)
    return 0;
  for (int i = 0; i < conca.get_size(); i++) {
    if (fabs(conca(i) - concb(i)) > zero_tolerance)
      return 0;
  }
    ...
    if (is_int(inv_acell * tb.cell)) {
      ...
      for (; shift; shift++) {
        ...
        for (at = 0; at < b.atom_pos.get_size(); at++) {
          ...
          if (at2 == -1)
            break;
          if (a.atom_type(at2) != b.atom_type(at))
            break;
        }
        if (at == b.atom_pos.get_size())
          return 1;
      }
    }
  }
...
void pointgroup_from_spacegroup(Array<rMatrix3d> *pointgroup,
  ...
  for (int op = 0; op < point_op.get_size(); op++) {
    ...
    while (i) {
      if (max_norm((*i) - point_op(op)) < zero_tolerance)
        break;
      i++;
    }
    if (!i) {
  ...
  for (int op = 0; op < point_op.get_size(); op++) {
    if (is_int((inv_cell)*point_op(op) * str.cell)) {
      ...
      for (; shift; shift++) {
        ...
        apply_symmetry(&transfo_pos, point_op(op), trans(op) + shift,
                       str.atom_pos);
        int at;
        for (at = 0; at < str.atom_pos.get_size(); at++) {
          if (str.atom_type(which_atom(str.atom_pos, transfo_pos(at),
                                       inv_cell)) != str.atom_type(at))
            break;
        }
...
void find_spacegroup(Array<rMatrix3d> *point_op, Array<rVector3d> *trans,
  ...
  int ref_atom = 0;
  while (atom_type(ref_atom) != rare_type) {
    ref_atom++;
  }
  Array<rMatrix3d> pointgroup;
  {
    ...
    if (atom_type(i) == rare_type) {
      for (int op = 0; op < pointgroup.get_size(); op++) {
        ...
        for (at = 0; at < atom_pos.get_size(); at++) {
          int at2 = which_atom(atom_pos, pointgroup(op) * atom_pos(at) + trans,
                               inv_cell);
          if (at2 < 0)
            break;
          if (atom_type(at2) != atom_type(at))
            break;
        }
...
void find_supercells(Array<rMatrix3d> *supercell, int min_volume,
  ...
  for (int volume = min_volume; volume <= max_volume; volume++) {
    ...
    MultiDimIterator<iVector3d> a(iVector3d(0, -imaxa, -imaxa),
                                  iVector3d(imaxa, imaxa, imaxa));
    for (; a; a++) {
      if (norm((iVector3d &)a) <= maxa + zero_tolerance) {
        ...
        while (cur_a) {
          ...
          for (sym = 0; sym < A.get_size(); sym++) {
            if ((A(sym) * (iVector3d &)a) == (*cur_a) ||
                (A(sym) * (iVector3d &)a) == -(*cur_a))
              break;
          }
          if (sym < A.get_size())
            break;
    ...
    for (; cur_a; cur_a++) {
      ...
      Real len_a = norm(a);
      Real maxb =
          sqrt(sqr(len_a) / 6. + sqrt(sqr(sqr(len_a) / 6.) +
                                      4. / 3. * sqr((Real)volume / len_a)));
      int imaxb = (int)ceil(maxb);
      MultiDimIterator<iVector3d> b(iVector3d(0, -imaxb, -imaxb),
                                    iVector3d(imaxb, imaxb, imaxb));
      for (; b; b++) {
        if (norm((iVector3d &)b) <= maxb + zero_tolerance &&
            norm((iVector3d &)a) >= len_a - zero_tolerance) {
          Real proj = (Real)((iVector3d &)b * (iVector3d &)a) /
                      (Real)((iVector3d &)a * (iVector3d &)a);
          iVector3d axb = (iVector3d &)a ^ (iVector3d &)b;
          int axb2 = norm2(axb);
          if (proj >= (-0.5 + zero_tolerance) &&
              proj <= (0.5 + 2. * zero_tolerance) && axb2 != 0) {
            int minw = -axb2 / 2;
            int maxw = -axb2 / 2 + axb2;
            for (int wa = minw; wa < maxw; wa++) {
              for (int wb = minw; wb < maxw; wb++) {
                iVector3d num =
                    volume * axb + wa * (iVector3d &)a + wb * (iVector3d &)b;
                int i;
                for (i = 0; i < 3; i++) {
                  if ((num(i) % axb2) != 0)
                    break;
                ...
                if (i == 3) {
                  ...
                  while (cur_abc) {
                    ...
                    for (sym = 0; sym < A.get_size(); sym++) {
                      ...
                      if (is_int((!new_abc) * old_abc) &&
                          is_int((!old_abc) * new_abc))
                        break;
                    }
                    if (sym < A.get_size())
                      break;
...
void find_supercells_2D(Array<rMatrix3d> *supercell, int min_volume,
  ...
  for (int volume = min_volume; volume <= max_volume; volume++) {
    ...
    int imaxa = (int)ceil(maxa);
    LinkedList<iVector3d> list_a;
    MultiDimIterator<iVector3d> a(iVector3d(0, -imaxa, 0),
                                  iVector3d(imaxa, imaxa, 0));
    for (; a; a++) {
      if (norm((iVector3d &)a) <= maxa + zero_tolerance) {
        LinkedListIterator<iVector3d> cur_a(list_a);
        while (cur_a) {
        ...
        if (i == 3) {
          ...
          while (cur_abc) {
            ...
            for (sym = 0; sym < A.get_size(); sym++) {
              rMatrix3d new_abc = to_real(A(sym) * abc);
              rMatrix3d old_abc = to_real(*cur_abc);
              if (is_int((!new_abc) * old_abc) && is_int((!old_abc) * new_abc))
                break;
            }
            if (sym < A.get_size())
              break;
...
void sort_supercells(Array<rMatrix3d> *p_supercell,
  ...
  for (int i = 0; i < nb_sym_op.get_size(); i++) {
    ...
    for (int j = 0; j < pointgroup.get_size(); j++) {
      if (is_int(icell * pointgroup(j) * cell))
        nb_sym_op(i)++;
    }
  }
...
int find_point_op_type(rVector3d *pspecial_dir, const rMatrix3d &point_op) {
  ...
  if (fabs(rdet - (Real)det) > zero_tolerance ||
      fabs(rtrace - (Real)trace) > zero_tolerance || abs(trace) > 3 ||
      abs(det) > 1) {
...
int number_of_point_op(int point_op_type, const Array<rMatrix3d> &point_op) {
  ...
  rVector3d dir;
  for (int op = 0; op < point_op.get_size(); op++) {
    if (find_point_op_type(&dir, point_op(op)) == point_op_type)
      nb++;
  }
  return nb;
}
...
const char *point_group_names[] = {
    "none",  "1",  "1_",  "2",   "m",    "2om",  "222",   "2mm", "mmm",
    "4",     "4_", "4om", "422", "4mm",  "4_2m", "4ommm", "3",   "3_",
    "32",    "3m", "3_m", "6",   "6_",   "6om",  "622",   "6mm", "6_m2",
    "6ommm", "23", "m3_", "432", "4_3m", "m3_m"};
...
void find_atom_point_group(Array<rMatrix3d> *patom_point_group,
  ...
  for (int op = 0; op < point_op.get_size(); op++) {
    if (equivalent_mod_cell(point_op(op) * atom_pos + trans(op), atom_pos,
                            inv_cell)) {
      int op2;
      for (op2 = 0; op2 < nb_op; op2++) {
        if (max_norm(point_op(op) - point_op(op_index(op2))) < zero_tolerance)
          break;
      }
      if (op2 == nb_op) {
        op_index(nb_op) = op;
...
int contains_pure_translations(const Array<rMatrix3d> &point_op,
  ...
  int nb_id = 0;
  for (int op = 0; op < point_op.get_size(); op++) {
    if (max_norm(point_op(op) - id) < zero_tolerance) {
      nb_id++;
    }
  }
  return (nb_id > 1);
  ...
  for (; l; l++) {
    if (!near_zero(norm(l))) {
      ...
      for (i = str.atom_pos.get_size() - 1; i >= 0; i--) {
        int j =
            which_atom(str.atom_pos, str.atom_pos(i) + (rVector3d)l, isupcel);
        if (str.atom_type(i) != str.atom_type(j))
          break;
      }
      if (i < 0)
        return 1;
  ...
  for (; l; l++) {
    for (int op = 0; op < point_op.get_size(); op++) {
      ...
      for (i = str.atom_pos.get_size() - 1; i >= 0; i--) {
        int j = which_atom(
            str.atom_pos,
            point_op(op) * str.atom_pos(i) + trans(op) + (rVector3d)l, isupcel);
        if (str.atom_type(i) > str.atom_type(j))
          return 1;
        if (str.atom_type(i) != str.atom_type(j))
          break;
...
class SymmetryOp {
public:
  SymmetryOp(const rMatrix3d &_point_op, const rVector3d &_trans)
      : point_op(_point_op), trans(_trans) {}
  rMatrix3d point_op;
  rVector3d trans;
};

void generate_space_group(SpaceGroup *p_fullgroup,
                          const SpaceGroup &generator) {
  ...
  while (!done) {
    ...
    for (; i; i++) {
      ...
      for (; j; j++) {
        MultiDimIterator<iVector3d> t(iVector3d(-2, -2, -2),
                                      iVector3d(2, 2, 2));
        for (; t; t++) {
          // cerr << to_real(t) << endl;
          SymmetryOp op(
              j->point_op * i->point_op,
              wrap_inside_cell(
                  j->point_op * (i->trans + generator.cell * to_real(t)) +
                      j->trans,
                  generator.cell));
          LinkedListIterator<SymmetryOp> k(sym_list);
          for (; k; k++) {
            //	  cerr << norm(op.point_op - k->point_op)/9 << " " <<
            //equivalent_mod_cell(op.trans,k->trans,invcell) << endl;
            if (norm(op.point_op - k->point_op) / 9 < zero_tolerance &&
                equivalent_mod_cell(op.trans, k->trans, invcell))
              break;
          }
          if (!k) {
...
void generate_point_group(Array<rMatrix3d> *p_fullgroup,
  ...
  while (1) {
    ...
    if (grpa.get_size() == grpb.get_size())
      break;
    grpa.delete_all();
