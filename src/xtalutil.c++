...
int in01included(rVector3d v) {
  for (int i = 0; i < 3; i++) {
    if (v(i) <= -zero_tolerance || v(i) >= 1 + zero_tolerance)
      return 0;
  }
  return 1;
}
...
int which_atom(const Array<rVector3d> &atom_pos, const rVector3d &pos) {
  for (int i = 0; i < atom_pos.get_size(); i++) {
    if (norm(atom_pos(i) - pos) < zero_tolerance)
      return i;
  }
  return -1;
}
...
void wrap_inside_cell(Array<rVector3d> *dest, const Array<rVector3d> &src,
...
void calc_lattice_vectors(rVector3d *cell_length, rVector3d *cell_angle,
                          const rMatrix3d &lattice_vector) {
  for (int i = 0; i < 3; i++) {
...
iVector3d find_sphere_bounding_box(const rMatrix3d &cell, Real radius) {
  ...
  for (int i = 0; i < 3; i++) {
    max_cell(i) =
        (int)ceil(norm(rec_lat.get_column(i)) * radius + zero_tolerance);
  }
  return (max_cell);
}
...
int equivalent_mod_cell(const Array<rVector3d> &a, const Array<rVector3d> &b,
                        const rMatrix3d &inv_cell, rVector3d *pshift) {
  ...
  for (int i = 0; i < a.get_size(); i++) {
    if (equivalent_mod_cell(a(0), b(i), inv_cell)) {
      ...
      for (j = 0; j < a.get_size(); j++) {
        if (which_atom(b, a(j) + shift) == -1)
          break; // if did not find match
      }
      if (j == a.get_size()) { // if all match
        if (pshift) {
          *pshift = shift;
        }
        return 1;
...
int equivalent_mod_cell(const MultiCluster &a, const MultiCluster &b,
                        const rMatrix3d &inv_cell) {
  if (a.clus.get_size() != b.clus.get_size())
    return 0;
  if (a.clus.get_size() == 0)
    return 1;
  for (int i = 0; i < a.clus.get_size(); i++) {
    if (equivalent_mod_cell(a.clus(0), b.clus(i), inv_cell)) {
      ...
      for (j = 0; j < a.clus.get_size(); j++) {
        ...
        if (at == -1)
          break; // if did not find matching site
        if (b.func(at) != a.func(j))
          break; // if did not find matching func
      }
      if (j == a.clus.get_size())
...
int equivalent_no_cell(const MultiCluster &a, const MultiCluster &b) {
  if (a.clus.get_size() != b.clus.get_size())
    return 0;
  if (a.clus.get_size() == 0)
    return 1;
  int j;
  for (j = 0; j < a.clus.get_size(); j++) {
    int at = which_atom(b.clus, a.clus(j));
    if (at == -1)
      break; // if did not find matching site
    if (b.func(at) != a.func(j))
      break; // if did not find matching func
...
void LatticePointInCellIterator::init(const rMatrix3d &_cell,
                                      const rMatrix3d &_supercell) {
  cell = _cell;
...
void LatticePointInCellIterator::advance_to_valid(void) {
  while (current) {
    if (in01(cell_to_super * to_real(current) + shift))
      break;
    current++;
  }
}
...
void find_all_atom_in_supercell(Array<rVector3d> *psuper_atom_pos,
                                ...
                                const rMatrix3d &cell,
                                const rMatrix3d &supercell) {
  int nb_atom_supercell =
      iround(atom_pos.get_size() * fabs(det(supercell) / det(cell)));
...
void find_all_atom_in_supercell(Array<rVector3d> *psuper_atom_pos,
                                ...
                                const Array<rVector3d> &atom_pos,
                                const Array<int> &atom_type,
                                const rMatrix3d &cell, const rMatrix3d
&supercell) { rMatrix3d super_to_cell=(!cell)*supercell; rVector3d take_tip;
  BoundingBox<Real,3> box;
  box.before_update();
  ...
  if (psuper_atom_type) super_atom_type.resize(nb_atom_supercell);
  {
    ...
    for (which_cell(0)=box.min(0); which_cell(0)<=box.max(0)+zero_tolerance;
which_cell(0)+=1.) { for (which_cell(1)=box.min(1);
which_cell(1)<=box.max(1)+zero_tolerance; which_cell(1)+=1.) { for
(which_cell(2)=box.min(2); which_cell(2)<=box.max(2)+zero_tolerance;
which_cell(2)+=1.) { for (int at=0; at<nb_atom; at++) { rVector3d
pos_axes=cell*which_cell+atom_pos(at); rVector3d
pos_super=axes_to_super*pos_axes; int i; for (i=0; i<3; i++) { if (pos_super(i)
< -zero_tolerance || pos_super(i) > 1.+zero_tolerance) break;
            }
            if (i==3) {
              int index2;
              for (index2=0; index2<index; index2++) {
                if
(equivalent_mod_cell(super_atom_pos(index2),pos_axes,axes_to_super)) break;
              }
              if (index2==index) {
                for (i=0; i<3; i++) {
    ...
    if (index!=super_atom_pos.get_size()) ERRORQUIT("Computed supercell does not
...
Real find_1nn_radius(const Structure &str, int atom) {
  ...
  while (1) {
    ...
    for (; i; i++) {
      if (cos_angle((*i)(1) - (*i)(0), pair(1) - (*i)(1)) > -zero_tolerance)
        break;
    }
    if (i) {
      break;
  ...
  for (int i = 0; i < atom_pos1->get_size(); i++) {
    for (int j = 0; j < atom_pos2->get_size(); j++) {
      ...
      if (minr < new_max_radius) {
        ...
        for (; norm(lat_vect) < (new_max_radius + minr + zero_tolerance);
             lat_vect++) {
...
#else
  LatticePointIterator lat_vect(cell, 0);
  for (; norm(lat_vect) < (new_max_radius + max_dist_in_cell + zero_tolerance);
       lat_vect++) {
    for (int i = 0; i < atom_pos1->get_size(); i++) {
      for (int j = (same_pos && norm(lat_vect) < zero_tolerance ? i : 0);
           j < atom_pos2->get_size(); j++) {
        rVector3d atom_a = (*atom_pos1)(i);
...
void AtomPairIterator::init(void) {
  ...
  for (int i = 0; i < atom_pos1->get_size(); i++) {
    for (int j = 0; j < atom_pos2->get_size(); j++) {
      max_dist_in_cell =
          MAX(max_dist_in_cell, norm((*atom_pos1)(i) - (*atom_pos2)(j)));
    }
  }
  inc_radius = MAXFLOAT;
  for (int i = 0; i < 3; i++) {
  ...
  while (it) {
    delete list.detach(it);
  }
  find_new_neighbors(inc_radius);
}

void AtomPairIterator::init(const rMatrix3d &_cell,
                            const Array<rVector3d> &_atom_pos) {
  cell = _cell;
  atom_pos1 = &_atom_pos;
  atom_pos2 = &_atom_pos;
  same_pos = 1;
...
void AtomPairIterator::init(const rMatrix3d &_cell,
                            const Array<rVector3d> &_atom_pos1,
                            const Array<rVector3d> &_atom_pos2) {
  cell = _cell;
...
void AtomPairIterator::init(const rMatrix3d &_cell, const rVector3d &_an_atom,
                            const Array<rVector3d> &_atom_pos) {
  cell = _cell;
  an_atom.resize(1);
  an_atom(0) = _an_atom;
...
void AtomMultipletIterator::init(const rMatrix3d &_cell,
                                 const Array<rVector3d> &_atom_pos,
                                 int _ntuple) {
  cell = _cell;
  atom_pos = &_atom_pos;
  ...
  while (1) {
    ...
    for (i = 0; i < pairs.get_size(); i++) {
      ...
      for (j = i + 1; j < pairs.get_size(); j++) {
        ...
        if (near_zero(r) || r < rmin - zero_tolerance ||
            r > rmax + zero_tolerance)
          break;
      }
      if (j < pairs.get_size())
        break;
...
void find_common_supercell(rMatrix3d *psupercell, const rMatrix3d &a,
                           const rMatrix3d &b) {
  rMatrix3d s, g;
  if (det(a) < det(b)) {
    s = a;
  ...
  v1 = p;
  while (!is_int(ig * p) ||
         fabs(sqr(v1 * p) - norm2(v1) * norm2(p)) < zero_tolerance)
    p++;
  v2 = p;
  while (!is_int(ig * p) || fabs((v1 ^ v2) * p) < zero_tolerance)
    p++;
  v3 = p;
...
void find_common_simple_supercell(iVector3d *psimple_supercell,
                                  const rMatrix3d &unitcell,
                                  const rMatrix3d &supercell) {
  rMatrix3d isupercell = !supercell;
  for (int i = 0; i < 3; i++) {
    int m = 1;
    while (!is_int(isupercell * ((Real)m * unitcell.get_column(i))))
      m++;
    (*psimple_supercell)(i) = m;
  }
}

void strain_str(Structure *pstr, const Structure &str,
                const rMatrix3d &strain) {
  rMatrix3d id;
  id.identity();
...
void apply_symmetry(Array<rVector3d> *b, const rMatrix3d &point_op,
                    const rVector3d &trans, const Array<rVector3d> &a) {
  b->resize(a.get_size());
  for (int i = 0; i < a.get_size(); i++) {
...
void apply_symmetry(MultiCluster *b, const rMatrix3d &point_op,
                    const rVector3d &trans, const MultiCluster &a) {
...
void reorder_atoms(Structure *rel_str, const Structure &str,
                   Array<int> *pcopy_from, Array<iVector3d> *pshift) {
  ...
  */
  ...
  for (int i = 0; i < n; i++) {
    rel_str->atom_pos(i) =
        tmpstr.atom_pos(copy_from(i)) + rel_str->cell * to_real(shift(i));
    rel_str->atom_type(i) = tmpstr.atom_type(copy_from(i));
  }
  if (pcopy_from)
    (*pcopy_from) = copy_from;
  if (pshift)
    (*pshift) = shift;
}
