...
int main(int argc, char *argv[]) {
  ...
  const char *corrfilename = "tcorr.out";
  int readcell = 0;
  int printcell = 0;
  int tp = 1;
  int ip = 0;
  ...
  AskStruct options[] = {
      {"",
       "GENerate Special Quasirandom Structures " MAPS_VERSION
      ...
      {"-tc",
       "Input file defining the target correlations (default: tcorr.out)",
       STRINGVAL, &corrfilename},
      ...
      {"-tp", "Total number of processes (for parallel operation)", INTVAL,
       &tp},
      {"-ip", "Index of current process (0,...,tp-1) (for parallel operation)",
       INTVAL, &ip},
  ...
  LinkedList<Real> corrlist;
  {
    ...
    if (!corrfile)
      ERRORQUIT("Unable to open target correlation file.");
  ...
  for (int c = bcell; c < ecell; c++) {
    ...
    find_all_atom_in_supercell(
        ...
        lat.atom_pos, lat.atom_type, lat.cell, blank_superstructure.cell);
    for (int i = 0; i < blank_superstructure.atom_type.get_size(); i++) {
      blank_superstructure.atom_type(i) =
          site_type_list(blank_superstructure.atom_type(i)).get_size();
    }
    ...
    for (; config; config++) {
      ...
      for (t = 0; t < corrarray.get_size(); t++) {
        if (fabs(corrarray(t) - calc_correlation(blank_superstructure,
                                                 eqclus(t), lat.cell,
                                                 *pcorrfunc)) > mysqstol)
          break;
      }
      if (t == corrarray.get_size()) {
        ...
        if (!contains_pure_translations(blank_superstructure, lat.cell)) {
          if (!equiv_to_lexico_successor(blank_superstructure, lat.cell,
                                         subgroup.point_op, subgroup.trans)) {
            if (tp > 1) {
              write_structure(blank_superstructure, lat, site_type_list,
                              atom_label, axes, outfile);
              outfile << "end" << endl << endl << flush;
            } else {
              write_structure(blank_superstructure, lat, site_type_list,
                              atom_label, axes, cout);
        ...
        if (!contains_pure_translations_or_lexico_successor(
                blank_superstructure, lat.cell)) {
          LinkedListIterator<Structure> is(strlist);
          for (; is; is++) {
            if (spacegroup(*is, blank_superstructure))
              break;
          }
          if (!is) {
