...
template <>
GenericPlugIn<ClusterExpansionCreator>
    *GenericPlugIn<ClusterExpansionCreator>::list = NULL;
SpecificPlugIn<ClusterExpansionCreator, ClusterExpansionCreator>
    StdClusterExpansionPlugIn("std"); // register plug-in under name "std";

Real calc_structure_cost(const Structure &str, const rMatrix3d &cell,
                         const Array<rMatrix3d> &point_op,
                         const Array<rVector3d> &trans, Real complexity_exp) {
  /*
  if (complexity_exp<0) {
...
ClusterExpansion::ClusterExpansion(const Structure &_parent_lattice,
    ...
    : pclusters(), equiv_cluster(), spacegroup(_spacegroup),
      ...
      parent_lattice(_parent_lattice), site_type_list(_site_type_list),
      atom_label(_atom_label), label_list(), i_label_list() {
  max_multiplet = 4;
  complexity_exp = 3.;
  giveup_weight = 16.;
  minc_gs_ok = -MAXFLOAT;
  ...
  for (nbsite = 0; nbsite < parent_lattice.atom_pos.get_size(); nbsite++) {
    if (site_type_list(parent_lattice.atom_type(nbsite)).get_size() == 1)
      break;
  }
  ...
  pclusters(0) =
      new ClusterBank(lattice_only.cell, lattice_only.atom_pos, 1, spacegroup);
  ...
  pclusters(1) =
      new ClusterBank(lattice_only.cell, lattice_only.atom_pos, 2, spacegroup);
  while (pclusters(1)->get_current_length() < radius_1nn) {
    (*pclusters(1))++;
  }
  ...
  for (int i = 0; i < pclusters.get_size(); i++) {
    ...
    for (; c; c++) {
      ...
      find_equivalent_clusters(parraycluster, *c, spacegroup.cell,
...
void ClusterExpansion::update_correlations(StructureInfo *str) {
  ...
  if (str->cost == 0)
    str->cost = calc_structure_cost(*str, spacegroup.cell, spacegroup.point_op,
                                    spacegroup.trans, complexity_exp);
  ...
  for (int clus_list = 0; clus_list < pclusters.get_size();
       clus_list++, corr_list++) {
    ...
    for (; idx < pclusters(clus_list)->get_current_index();
         clus++, equiv_clus++, idx++) {
      (*corr_list) << new Real(
          calc_correlation(*str, *equiv_clus, spacegroup.cell));
    }
  }
...
void ClusterExpansion::calc_point_to_concentration(Array<Real> *multiplicity,
                                                   int nb_cols) {
  ...
  for (int j = 1; i; i++, j++) {
    (*multiplicity)(j) =
        (Real)calc_multiplicity(*i, lattice_only.cell, spacegroup.point_op,
                                spacegroup.trans) /
        (Real)lattice_only.atom_pos.get_size();
  }
}

void ClusterExpansion::calc_concentration(Array<Real> *pconcentration,
                                          const Array2d<Real> &corr_matrix) {
  // figure out the weight to give to each point corr to get the concentration;
...
void ClusterExpansion::calc_regression_matrices(Array2d<Real> *pcorr_matrix,
                                                Array<Real> *penergy,
...
void ClusterExpansion::calc_predictor_energy(Array<Real> *penergy,
                                             StructureInfo::Status select) {
  // update all selected structures;
  update_correlations(select);
  // first count the number of structures selected and resize relevant arrays;
  ...
  for (; str; str++) {
    if (str->status & select) { // if structure matches mask;
      (*penergy)(i_str) = 0.;
      LinkedListIterator<EnergyPredictor> ip(
          str->predictor); // loop through user-defined predictor;
      for (; ip; ip++) {
        (*penergy)(i_str) += ip->get_energy(*str);
      }
      // cout << str->label << " " << (*penergy)(i_str) << endl; // trace info,
      // if needed;
      i_str++;
    }
  }
...
void ClusterExpansion::reset_cluster_choice(void) {
  ...
  LinkedListIterator<StructureInfo> str(structures.get_structure_list());
  for (; str; str++) {
    if (str->status == StructureInfo::calculated)
      nb_calculated_str++;
  }
  // init nb of clusters in fit;
  nb_clusters =
      1 + pclusters(0)->get_current_index() + pclusters(1)->get_current_index();
...
void ClusterExpansion::update_equivalent_clusters(void) {
  // update the lists of clusters equivalent by a space group symmetry operation
  // (to speed up the calculation of the correlations);
  ...
  for (int m = 0; m < equiv_cluster.get_size(); m++) {
    ...
    for (; c; c++) { // loop through clusters;
      ...
      find_equivalent_clusters(pequiv, *c, spacegroup.cell, spacegroup.point_op,
                               spacegroup.trans);
...
int ClusterExpansion::find_next_cluster_choice(void) {
  ...
  if (pclusters(pclusters.get_size() - 1)->get_current_index() > 0 &&
      pclusters.get_size() < max_multiplet) {
    Array<ClusterBank *> tmp(pclusters);
    pclusters.resize(tmp.get_size() + 1);
    for (int i = 0; i < tmp.get_size(); i++) {
      pclusters(i) = tmp(i);
    }
    pclusters(tmp.get_size()) =
        new ClusterBank(*(pclusters(1)), tmp.get_size() + 1);
  }
  ...
  int c = 0;
  // A;
  while (c < pclusters.get_size() - 1 &&
         pclusters(c)->get_current_index() > 0) {
    c++;
  }
  // B;
  while (1) {
    ...
    if (nb_clusters < nb_calculated_str &&
        pclusters(c)->get_previous_length() <
            pclusters(c - 1)->get_previous_length() + zero_tolerance)
      break; // E;
    // F;
  ...
  // update the lists of clusters equivalent under space group symmetry
  // operation;
...
void ClusterExpansion::set_cluster_choice(const Array<int> &choice) {
  ...
  for (i = 1; i < choice.get_size(); i++) {
    pclusters(i)->reset();
    while (pclusters(i)->get_current_index() < choice(i))
      (*pclusters(i))++;
...
void ClusterExpansion::find_best_cluster_choice(CEFitInfo *pfitinfo) {
  ...
  if (corr_matrix.get_size()(0) <=
      corr_matrix.get_size()(1)) { // enough structures?;
    if (pfitinfo) {
      pfitinfo->status = CEFitInfo::fit_impossible;
    }
    return;
  }
  if (pfitinfo) {
    pfitinfo->status = CEFitInfo::fit_ok;
  }
  ...
  Array<int> best_choice_noweight; // if weighting fails, save the best noweight
                                   // solution;
  ...
  // if a user specified choice has been given;
  {
    ...
    if (cluster_choice_file) {
      ...
      cerr << "nbclusters.in file detected, using user-specified cluster choice"
           << endl;
  ...
  if (best_choice.get_size() > 0) { // if user-specified cluster choice;
    ...
    while (1) {               // repeat until ground states ok;
      ...
      do { // loop through cluster choices;
        ...
        Array<int> problems;
        int gs_ok =
            !ground_state_problem(&problems, true_gs, concentration,
                                  fitted_energy, minc_gs_ok, maxc_gs_ok);

        // save best choice so far;
        cerr << gs_ok << " " << cv << endl;
        if ((gs_ok > best_gs_ok && cv != MAXFLOAT) ||
            (gs_ok == best_gs_ok && cv < best_cv)) {
          best_gs_ok = gs_ok;
      ...
      // check for qualitatively wrong ground states;
      Array<int> problems;
      if (!ground_state_problem(&problems, true_gs, concentration,
                                fitted_energy, minc_gs_ok, maxc_gs_ok))
        break; // no problems: exit loop;
  ...
  set_cluster_choice(
      best_choice);         // save best choice into ClusterExpansion object;
  ...
  if (pfitinfo) {
    ...
    calc_formation(&energy, concentration, energy, &(pfitinfo->pure_energy(0)),
                   &(pfitinfo->pure_energy(1)));
    diff(&ce_energy, energy, predictor_energy);
    ...
    // predicted ground state line (among structures whose energy is known)
    // using fitted energy (not used later on);
    calc_convex_hull(&(pfitinfo->fitted_gs_list), concentration,
                     pfitinfo->fitted_energy);
    Array<int> problems;
    if (ground_state_problem(&problems, pfitinfo->fitted_gs_list, concentration,
                             pfitinfo->fitted_energy, minc_gs_ok, maxc_gs_ok)) {
      pfitinfo->status = CEFitInfo::gs_problem;
    }
    ...
    // of unknown energies;
    {
      ...
      convex_hull_from_gs(&gs_concentration, &gs_fitted_energy,
                          pfitinfo->gs_list, concentration,
                          pfitinfo->fitted_energy);
      ...
      calc_regression_matrices(&all_corr, NULL,
                               (StructureInfo::Status)(StructureInfo::unknown |
                                                       StructureInfo::busy |
                                                       StructureInfo::error));
      calc_predictor_energy(&all_predictor_energy,
                            (StructureInfo::Status)(StructureInfo::unknown |
                                                    StructureInfo::busy |
                                                    StructureInfo::error));
      ...
      Array<int> problem;
      if (ground_state_problem(&problem, gs_concentration, gs_fitted_energy,
                               all_concentration, all_fitted_energy, minc_gs_ok,
                               maxc_gs_ok)) {
        pfitinfo->status =
            (CEFitInfo::Status)(pfitinfo->status | CEFitInfo::new_gs);
      }
      // output all conc. and energy of all generated structures of unknown
      // energy;
      {
        LinkedListIterator<StructureInfo> s(structures.get_structure_list());
        ...
        for (int i = 0; i < all_fitted_energy.get_size(); i++) {
          while (!(s->status & (StructureInfo::Status)(StructureInfo::unknown |
                                                       StructureInfo::busy |
                                                       StructureInfo::error))) {
            s++;
          }
          ...
          Real form_e = s->energy / volume -
                        ...
                        c * pfitinfo->pure_energy(1);
          allstr << c << " " << (s->energy == 0 ? 0 : form_e) << " "
                 << all_fitted_energy(i) << " "
    ...
    for (int m = 0; m < best_choice.get_size(); m++) {
      ...
      for (int i = 0; i < best_choice(m); i++, (*pclusters(m))++, c++) {
        ...
        pfitinfo->multiplicity(c) =
            (Real)calc_multiplicity(pfitinfo->cluster(c), parent_lattice.cell,
                                    spacegroup.point_op, spacegroup.trans);
...
void ClusterExpansion::make_correlation_array(
    Array<Real> *array_corr, const LinkedList<LinkedListReal> &list_corr) {
...
StructureInfo *ClusterExpansion::find_best_structure(void) {
  ...
  if (corr.get_size()(1) < 3 || corr.get_size()(0) < corr.get_size()(1))
    return NULL;
  ...
  Array<Real> gs_concentration;
  calc_convex_hull(&true_gs, concentration, energy);
  convex_hull_from_gs(&gs_concentration, &gs_fitted_energy, true_gs,
                      concentration, fitted_energy);
  ...
  if (ground_state_problem(&problem, gs_concentration, gs_fitted_energy,
                           un_concentration, un_fitted_energy, minc_gs_ok,
                           maxc_gs_ok)) {
    // if there are new predicted ground state, look among them for the
    // structure that give the greatest var reduction;
    Array2d<Real> rhorho;
    ...
    for (int i = 0; i < new_gs.get_size(); i++) {
      problem(new_gs(i)) |=
          2; // structures that break the hull and are potential new ground
             // states will be labelled 3;
    }
    ...
    Real max_benefit =
        0.; // benefit is variance reduction over computational cost;
    reset_structure();
    // loop over predicted ground states whose true energy is unknown;
    for (int istr = 0; istr < problem.get_size(); istr++) {
      while (structures.get_current_structure().status !=
             StructureInfo::unknown) {
        find_next_structure();
      }
      if (problem(istr) == 3) { // if new structure breaks the hull and is a
                                // potential new ground state;
        // calculate variance after the current structure has been added to the
        // fit;
        Real var;
        {
          Array<Real> raw_cur_rho, cur_rho;
          make_correlation_array(
              &raw_cur_rho, structures.get_current_structure().correlations);
        ...
        Real benefit =
            (var_before - var) / structures.get_current_structure().cost;
      ...
      if (istr < problem.get_size() - 1)
        find_next_structure();
    } // loop to next structure;
    return p_best_str;
  } else {
    // no ground state problems: look among all unknown structures for the most
    // variance-reducing one;
    Array2d<Real> rhorho;
    // treat all structures as equally important;
    set_identity(&rhorho, corr.get_size()(1));
    ...
    {
      // best_dir is the eigenvector associated with the smallest eigenvalue of
      // xw2x; find by iterating u=inverse(xw2x)*v; v=u/|u|;
      zero_array(&best_dir);
    ...
    // now we have the maximum possible variance reduction due to adding a
    // structure;
    ...
    while (1) {
      if (structures.get_current_structure().status == StructureInfo::unknown) {
        ...
        if (max_possible_dvar / structures.get_current_structure().cost <
                max_benefit ||
            structures.get_current_structure().atom_pos.get_size() >
                user_max_vol)
          break;
        // calculate variance after the current structure has been added to the
        // fit;
        ...
        Real cur_concentration;
        {
          ...
          calc_point_to_concentration(&point_multiplicity,
                                      raw_cur_rho.get_size());
...
StructureInfo *ClusterExpansion::find_initial_structures(void) {
  ...
  while (1) {
    if (structures.get_current_structure().status == StructureInfo::unknown) {
      Array<Real> cur_corr;
      make_correlation_array(&cur_corr,
                             structures.get_current_structure().correlations);
      Array<Real> point_multiplicity;
      ...
      if (c >= minc_gs_ok && c <= maxc_gs_ok) {
        ...
        if (build_basis(&basis, &nb_in_basis, cur_corr))
          break;
      }
    }
    find_next_structure();
