...
void distord_cell(rMatrix3d *pcell, const rVector3d &dir, Real e_paral,
		  Real e_perp) {
...
int main(int argc, char *argv[]) {
  ...
  const char *puredir[2] = {"0", "1"};
  const char *dirfilename = "dir.in";
  const char *latticefilename = "lat.in";
  int dummy = 0;
  int sigdig = 6;
  int polltime = 10;
  AskStruct options[] = {
      {"", "Constituent Strain FITter " MAPS_VERSION ", by Axel van de Walle",
       TITLEVAL, NULL},
      {"-h", "Display more help", BOOLVAL, &dohelp},
      {"-nc", "Number of points in concentration mesh (default 50)", INTVAL,
       &conc_mesh},
      {"-ns",
       "Number of points in mesh used to look for energy minimum (default 100)",
       INTVAL, &search_mesh},
      {"-np",
       "Number of points in stretching mesh in the direction perpendicular to "
       "the k-vector(default 5)",
       INTVAL, &perp_stretch_mesh},
      {"-nl",
       "Number of points in stretching mesh in the direction parallel to the "
       "k-vector (default 3)",
       INTVAL, &paral_stretch_mesh},
      {"-ml", "Maximum parallel stretching (default 0.05)", REALVAL,
       &max_paral_stretch},
      {"-l", "Input file defining the lattice (default: lat.in)", STRINGVAL,
       &latticefilename},
      {"-pa", "Directory containing the pure A calculations (default 0/)",
       STRINGVAL, &puredir[0]},
      {"-pb", "Directory containing the pure B calculations (default 1/)",
       STRINGVAL, &puredir[1]},
      {"-ds",
       "File containing a list of stretching directions (default: dir.in)",
       STRINGVAL, &dirfilename},
  ...
  // read in the lattice file (so that we know the conventional cell and
  ...
  for (int elem = 0; elem < 2; elem++) {
    ...
    }
    {
      ...
      if (!file) {
	cerr << "Unable to open file" << puredir[elem] << "/str-relax.out";
	ERRORQUIT("");
      }
      parse_structure_file(&str[elem].cell, &str[elem].atom_pos,
			   &str[elem].atom_type, atom_label, file);
    }
    scale[elem] =
  ...
  // generate distorted structures;
  {
	...
	for (int perp = 0; perp < perp_stretch_mesh; perp++) {
	  ...
	  Real stretch_perp =
	      (1. - t) * (scale[0] / scale[elem]) +
	      t * (scale[1] / scale[elem]);  // interpolate between lattice
					     // parameter of each end member;
	  for (int paral = 0; paral < paral_stretch_mesh; paral++) {
	    // calc the stretching along k vector: take volume preserving value
	    // and add or substract a fraction of maximum parallel strecthing;
	    Real stretch_paral =
		pow(stretch_perp, -2.) +
		(2. * (Real)paral / (Real)(paral_stretch_mesh - 1) - 1.) *
		    max_paral_stretch;
	    Structure stretched_str(str[elem]);
	    rVector3d dir = rec_axes * (*i_dir);
	    distord_cell(&(stretched_str.cell), dir, stretch_paral,
			 stretch_perp);
	    rMatrix3d transfo = stretched_str.cell * (!str[elem].cell);
	    for (int i = 0; i < str[elem].atom_pos.get_size(); i++) {
	      stretched_str.atom_pos(i) = transfo * str[elem].atom_pos(i);
	    }
	    ...
	    }
	    {
	      ...
	      write_structure(stretched_str, atom_label,
			      transfo * scale[elem] * axes, file);
	    }
  ...
  while (1) {
	...
	for (int perp = 0; nomissing && perp < perp_stretch_mesh; perp++) {
	  for (int paral = 0; nomissing && paral < paral_stretch_mesh;
		...
		cerr << "Error reading energy file in " << cur_dir.str().c_str()
		     << endl;
		nomissing = 0;
	      }
	    }
  ...
  for (int n_kubic = 2; n_kubic <= MIN(4, dirlist.get_size() - 1); n_kubic++) {
	...
	cs_e(index_dir) =
	    find_minimum(perp_e, search_mesh) -
	    (pure_e[0](index_dir) * (1. - c) + pure_e[1](index_dir) * c);
	file << cs_e(index_dir) << " ";
