...
int get_values(istream &s, int nb, AskStruct *label) {
  ...
  while (1) {
    while (1) {
      c = s.get();
      if (!strchr(blanks_delim, c) || s.eof())
        break;
    }
    if (s.eof())
      return 1;
  ...
  for (int i = 0; i < nb; i++) {
    if (label[i].vartype == TITLEVAL) {
      ...
      case REALVAL:
        ...
        break;
      case BOOLVAL:
        cerr << "            " << label[i].longname
             << " (Default: " << (*(int *)label[i].outvar != 0 ? "On" : "Off")
             << ")" << endl;
        break;
      case STRINGVAL:
      ...
      case ARRAYRVAL:
        ...
        if (dodef) {
          cerr << " (Default: ";
          for (int j = 0; j < ((Array<Real> *)(label[i].outvar))->get_size();
               j++) {
            if (j > 0) {
              cerr << ",";
            }
...
int get_string(AutoString *ps, istream &file, const char *delim) {
  ...
  while (1) {
    file.get(c);
    if (strchr(delim, c) != NULL || file.eof())
      break;
    accum += c;
  }
  *ps = accum;
