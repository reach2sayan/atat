...
int isnumber(char c) { return (isdigit(c) || strchr("-+.", c)); }
...
void parse_lattice_file(rMatrix3d *pcell, Array<rVector3d> *patom_pos,
  ...
  while (1) {
    ...
    file >> pos;
    if (file.eof() || pos(0) == MAXFLOAT || !file)
      break;
    pos = axes * pos;
    ...
    while (atom_begin < string_end) {
      while (strchr(delim, *atom_begin) && atom_begin < string_end) {
        atom_begin++;
      }
      if (atom_begin == string_end)
        break;
      char *atom_end = atom_begin;
      while (!strchr(delim, *atom_end) && atom_begin < string_end) {
        atom_end++;
      }
      *atom_end = 0;
      /*
  ...
  while (1) {
    ...
    while (atom_begin < string_end) {
      ...
      if (atom_begin == string_end)
        break;
      char *atom_end = atom_begin;
      while (!strchr(eqdelim, *atom_end) && atom_begin < string_end) {
        atom_end++;
      }
      *atom_end = 0;
      site_types << new AutoString(atom_begin);
      ...
      while (strchr(numbersdot, *atom_end) && atom_begin < string_end) {
        atom_end++;
      }
      *atom_end = 0;
      istringstream probstr(atom_begin);
      Real prob = -1;
      probstr >> prob;
      if (prob == -1) {
        prob = 1.;
...
int parse_structure_file(rMatrix3d *pcell, Array<rVector3d> *patom_pos,
  ...
  while (1) {
    rVector3d pos(MAXFLOAT, MAXFLOAT, MAXFLOAT);
    file >> pos;
    if (file.eof() || pos(1) == MAXFLOAT || !file)
      break;
    pos = axes * pos;
    ...
    if (file.eof()) {
      ERRORQUIT("Unexpected end of file in structure file");
    }
    file.putback(c);
    char buf[MAX_LINE_LEN];
    file.get(buf, MAX_LINE_LEN - 1);
    char *end_label = buf;
    while (end_label < buf + strlen(buf) && !strchr(" \t", *end_label)) {
      end_label++;
    }
    *end_label = 0;
    int i;
...
int fix_atom_type(Structure *pstr, const Structure &lat,
  ...
  for (int i = 0; i < pstr->atom_type.get_size(); i++) {
    ...
    if (j == site_type_list(site_type).get_size())
      return 0;
  }
  if (drop_fixed_site) {
    ...
    for (int i = 0; i < pstr->atom_pos.get_size(); i++) {
      if (site_type_list(lat.atom_type(which_atom(
                             lat.atom_pos, pstr->atom_pos(i), !lat.cell)))
              .get_size() > 1) {
...
int fix_atom_type(Structure *pstr, const Array<Arrayint> &site_type_list) {
  int bad = 0;
  for (int i = 0; i < pstr->atom_type.get_size(); i++) {
    if (site_type_list(pstr->atom_type(i)).get_size() != 1) {
      bad = 1;
    }
...
void write_structure(const Structure &str, const Structure &lat,
  ...
  for (int i = 0; i < str.atom_pos.get_size(); i++) {
    ...
    file << (iaxes * str.atom_pos(i)) << " "
         << atom_label(
                site_type_list(lat.atom_type(site_in_cell))(str.atom_type(i)))
         << endl;
  }
}
