 ...
 *       Filename:  apb.cc
 *
 *    Description:  This code creates an APB for a given structure and slip
 * vector. The APB energy is calculated by calling memc2 with clusters and ECIs
 * from a prior cluster expansion.
 *
...
int main(int argc, char *argv[]) {
  ...
  AskStruct options[] = {
      {"",
       "\033[1;4mA\033[0mnti\033[1;4mP\033[0mhase "
       "\033[1;4mB\033[0moundary " MAPS_VERSION
       ", by Ruoshi Sun and Axel van de Walle\nAutomating impurity-enhanced "
       "antiphase boundary energy calculations from ab initio Monte Carlo,\n "
       "Calphad 53, 20 (2016)\n\n\033[1mFile options:\033[0m",
       TITLEVAL, NULL},
      {"-l", "Input file: lattice   (Default: lat.in)", STRINGVAL,
       &latfilename},
      {"-s", "          : structure (Default: str.out)", STRINGVAL,
       &strfilename},
      {"-o", "Output file: APB structure (Default: str_apb.out)", STRINGVAL,
       &apbfilename},
      {"-og",
       "           : energies      (Default: gamma_apb.out)\n\n\033[1mAPB "
       "options:\033[0m",
       STRINGVAL, &gammafilename},
      {"-f", "Generate APB structure file and exit; do not compute APB energy",
       BOOLVAL, &dofileonly},
  ...
  if (!clusterfile)
    ERRORQUIT("Unable to open clusters.out");
  ifstream ecifile(ecifilename);
  if (!ecifile)
    ERRORQUIT("Unable to open eci.out");
  ifstream gsfile(gsfilename);
  if (!gsfile)
    ERRORQUIT("Unable to open gs_str.out");
  ...
  apb_energy(strfilename, apbfilename, gammafilename, area, n_atom, n_type, nf,
             false, 0);
  ...
  if (domc) {
    ...
    for (int i = 0; i != n_type; ++i) {
      concfile << "1.0*" << label[i] << " = " << n_atom_type[i] / n_atom
               << endl;
    }
    ...
    snprintf(memc2_command, 200,
             "memc2 -cf=%s -is=%s -n=%d -eq=%d -opss=str0000.out -keV -q",
             controlfilename, strfilename, nav, neq);
    ...
    for (int i = 0; i != nav; ++i) {
      ...
      wrap_inside_cell(&str.atom_pos, str.atom_pos, str.cell);
      write_apb_structure(str, label, axes, apbfile, slipvec);
      apb_energy(strf, apbf, gammafilename, area, n_atom, n_type, nf, domc, i);
    }
  }
 ...
 * ===  FUNCTION
 * ====================================================================== Name:
 * write_apb_structure Description:  Write APB structure file. Duplicate atoms
 * along T3 and shift them by APB slip vector.
 * =====================================================================================
 */
void write_apb_structure(const Structure &str,
  ...
  for (int i = 0; i != str.atom_pos.get_size(); ++i) {
    file << iaxes * str.atom_pos(i) << " " << atom_label(str.atom_type(i))
         << endl;
 ...
 * ===  FUNCTION
 * ====================================================================== Name:
 * get_area Description:  The cross-sectional area of the supercell is given by
 * ||T1 x T2||.
 * =====================================================================================
 */
 ...
 * ===  FUNCTION
 * ====================================================================== Name:
 * write_control Description:  Write control.in
 * =====================================================================================
 */
void write_control(const Real &T, const int &n, const int &n_type,
 ...
 * ===  FUNCTION
 * ====================================================================== Name:
 * get_energy Description:  Obtain the total energy (eV/atom) of a structure
 * using ECIs via memc2.
 * =====================================================================================
 */
Real get_energy(const char *strfilename, int &nf, const int &n_type,
  ...
  char memc2_command[200];
  snprintf(memc2_command, 200, "memc2 -cf=%s -is=%s -o=%s -n=0 -eq=0 -g2c -q",
           controlfilename, strfilename, mcfilename);
  system(memc2_command);

  ifstream mcfile(mcfilename);
  if (!mcfile)
    ERRORQUIT("Unable to open monte carlo output file");
  ...
  if (nf == 0) {
    ...
    if (!found)
      ERRORQUIT("Cannot find E_mc in mcheader.out");
  }
 ...
 * ===  FUNCTION
 * ====================================================================== Name:
 * apb_energy Description:  Compute APB energy, defined as 2N*e1 - 2N*e0
 * (e1-e0)*N
 *                    ------------- = ---------
 *                         2*A            A
 ...
 * =====================================================================================
 */
void apb_energy(const char *strfilename, const char *apbfilename,
                const char *gammafilename, const Real &area, const int &n_atom,
                const int &n_type, int &nf, bool domc, const int &i) {
  ofstream gammafile;
  if (domc && i != 0) {
  ...
  Real e0 = get_energy(strfilename, nf, n_type, "energy_mc.out");
  Real e1 = get_energy(apbfilename, nf, n_type, "energy_mc_apb.out");
