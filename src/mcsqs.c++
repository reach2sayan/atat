...
void calc_all_corr(Array<Real> *pcorr, const Structure &str,
                   const Array<Array<MultiCluster>> &eqclus,
...
/*
Real calc_objective_func(const Array<Real> &corr, const Array<Real> &tcorr, Real
mysqstol, Real weightdist, const Array<Real> &diam, const Array<int> &nbpt) {
...
Real calc_objective_func(const Array<Real> &corr, const Array<Real> &tcorr,
                         Real mysqstol, Real weightdist, Real weightnbpt,
                         Real weightdecay, const Array<Real> &diam,
                         const Array<int> &nbpt) {
  Array<Real> dcorr(tcorr.get_size());
...
void open_numbered_file(ofstream &file, const char *prefix, int num,
                        const char *suffix) {
  ...
  if (!file) {
    cerr << "Unable to open " << filename.str().c_str() << endl;
    ERRORQUIT("Aborting");
  }
}
...
int main(int argc, char *argv[]) {
  ...
  zero_array(&maxd);
  const char *rndstrfilename = "rndstr.in";
  const char *clusterfilename = "clusters.out";
  const char *tcorrfilename = "";
  const char *paramfilename = "sqsparam.in";
  int readcell = 0;
  int ip = -1;
  int findbest = 0;
  ...
  AskStruct options[] = {
      {"",
       "Monte Carlo generator of Special Quasirandom Structures " MAPS_VERSION
       ", by Axel van de Walle",
       TITLEVAL, NULL},
      {"-n", "nb of atom/unit cell", INTVAL, &maxvol},
      {"", "If -n is not specified, generate clusters of the following sizes:",
       TITLEVAL, NULL},
      {"-2", " Maximum distance between two points within a pair", REALVAL,
       &maxd(2)},
      {"-3", " Maximum distance between two points within a triplet", REALVAL,
       &maxd(3)},
      {"-4", " Maximum distance between two points within a quadruplet",
       REALVAL, &maxd(4)},
      {"-5", " Maximum distance between two points within a quintuplet",
       REALVAL, &maxd(5)},
      {"-6", " Maximum distance between two points within a sextuplet", REALVAL,
       &maxd(6)},
      {"-l", "Input file defining the random structure (default: rndstr.in)",
       STRINGVAL, &rndstrfilename},
      {"-cf", "Input file defining the clusters (default: clusters.out)",
       STRINGVAL, &clusterfilename},
      {"-tcf",
       "Input file defining the target multibody correlations (default: "
       "internally calculated values for fully disordered state)",
       STRINGVAL, &tcorrfilename},
      {"-tol", "Tolerance for matching correlations (default: 1e-3)", REALVAL,
       &mysqstol},
      {"-wr",
       ...
       REALVAL, &weightdist},
      {"-wn",
       "Multiplicative decrease in weight per additional point in cluster "
       "(default 1)",
       REALVAL, &weightnbpt},
      {"-wd",
       "Exponent of decay in weight as function of cluster diameter (default "
       "0)",
       REALVAL, &weightdecay},
      {"-T", "Temperature (default 1)", REALVAL, &T},
      {"-pf",
       "Input file defining the optimization parameters (default: sqsparam.in)",
       STRINGVAL, &paramfilename},
      {"-rc",
       "Read supercells from file sqscell.out (default: generate internally "
       "and write to sqscell.out)",
       BOOLVAL, &readcell},
      {"-ip", "Index of current process (for parallel operation)", INTVAL, &ip},
      {"-best", "Collect best SQS among the outputs of prior parallel runs",
       BOOLVAL, &findbest},
      {"-crf", "Select correlation functions (default: trigo)", STRINGVAL,
       &corrfunc_label},
      {"-sd", "Seed for random number generation (default: use clock)", INTVAL,
       &seed},
      {"-rt", "Read parameter file every (rt) step (default:10000)", INTVAL,
       &maxtic},
      {"-2d", "Generate only supercells in the plane of a,b axes", BOOLVAL,
       &do2d},
      {"-sig",
       "Number of significant digits to print in output files (default: 6)",
       INTVAL, &sigdig},
      {"-h", "Display more help", BOOLVAL, &dohelp}};
  if (!get_values(argc, argv, countof(options), options)) {
    display_help(countof(options), options);
    if (argc == 1) {
      cerr << endl << "Perhaps missing -n=[nb of atom/cell] option?" << endl;
    }
    return 1;
  }
  if (dohelp) {
  ...
  if (findbest) {
    {
      ...
      script << "#!/bin/sh" << endl
             << "stem=$(grep -n Objective bestcorr?*.out | sort -n -k 2 | head "
                "-1 | sed 's/:.*$//g')"
             << endl
             << "if [ \"${stem}\" == \"\" ]; then" << endl
  ...
  if (maxvol == 0 && !readcell) {
    ERRORQUIT("Please specify -n option.");
  }
  ...
  rMatrix3d axes;
  {
    ...
    if (!file)
      ERRORQUIT("Unable to open random structure file.");
    parse_rndstr_file(&ulat.cell, &ulat.atom_pos, &ulat.atom_type, &uatomprob,
  ...
  find_spacegroup(&spacegroup.point_op, &spacegroup.trans, ulat.cell,
                  ulat.atom_pos, ulat.atom_type);
  ...
  Array<Array<Real>> sym_type_prob;
  {
    ...
    for (int symt = 0; i_pt_clus; i_pt_clus++, symt++) {
      ...
      find_equivalent_clusters(&pt_clus_eq, *i_pt_clus, spacegroup.cell,
      ...
      for (int eq = 0; eq < pt_clus_eq.get_size(); eq++) {
        ...
        if (norm(dprob) > zero_tolerance) {
          cerr << "Different occupations assigned to symmetrically equivalent "
                  "site:"
               << endl;
          cerr << inv_axes * (*i_pt_clus)(0) << endl
               << "and" << endl
               << inv_axes * pt_clus_eq(eq)(0) << endl;
          ERRORQUIT("Aborting.");
        }
        at++;
      }
  ...
  if (ip <= 0) {
    ...
    for (int i = 0; i < lat.atom_pos.get_size(); i++) {
      if (last_type != lat.atom_type(i)) {
        groupfile << endl;
      }
      last_type = lat.atom_type(i);
      groupfile << (inv_axes * lat.atom_pos(i)) << " ";
      //      int
      //      site_type=ulat.atom_type(which_atom(ulat.atom_pos,lat.atom_pos(i),inv_cell));
      int site_type = sym_to_type(lat.atom_type(i));
      for (int j = 0; j < site_type_list(site_type).get_size(); j++) {
        if (j != 0) {
          groupfile << ",";
        }
        groupfile << atom_label(site_type_list(site_type)(j)) << "="
                  << sym_type_prob(lat.atom_type(i))(j);
      }
      groupfile << endl;
    }
  }
  ...
  for (int at = 0; at < lat.atom_type.get_size(); at++) {
    ...
    for (int func = 0; func < comp - 1; func++) {
      ...
      for (int p = 0; p < atomprob(at).get_size(); p++) {
        ...
        if (atomprob(at)(p) < 0) {
          ERRORQUIT("Negative occupations!");
        }
        if (atomprob(at).get_size() > 1 && near_zero(atomprob(at)(p) - 1)) {
          ERRORQUIT("Please drop irrelevant species when site is fully "
                    "occupied with one specie");
        }
        totprob += atomprob(at)(p);
      }
      if (!near_zero(totprob - 1.))
        ERRORQUIT("Probabilities must sum to 1.");
  ...
  LinkedList<MultiCluster> clusterlist;
  {
    ...
    if (!clusterfile)
      ERRORQUIT("Unable to open cluster file.  Please generate it with\n mcsqs "
                "-2=[max pair diameter] -3=[max triplet diameter] etc.");
  ...
  if (strlen(tcorrfilename) > 0) {
    ...
    if (!tcorrfile) {
      ERRORQUIT("Unable to open correlation file");
    }
  }
    ...
    if (icluster->clus.get_size() <= 1) {
      ...
      if (strlen(tcorrfilename) > 0) {
        ...
        for (int i = 0; i < icluster->clus.get_size(); i++) {
          ...
          if (at == -1) {
            ERRORQUIT("Cluster file inconsistent with random structure file.");
          }
  ...
  for (int t = 0; ic; t++, ic++) {
    find_equivalent_clusters(&eqclus(t), *ic, lat.cell, spacegroup.point_op,
                             spacegroup.trans);
  ...
  if (readcell) {
    ...
    if (ip <= 0) {
      ...
      for (int i = 0; i < supercell.get_size(); i++) {
        // cout << supercell(i) << " ";
        // cout << shorten_lattice_vectors(supercell(i)) << endl;
        supercell(i) = find_symmetric_cell(supercell(i));
        write_axes(iaxes * supercell(i), cellfile, 0);
        cellfile << endl;
      }
    } else {
      logfile << "Not writing supercells (process -ip=0 is doing that)." << endl
              << flush;
    }
  }
  ...
  for (int c = 0; c < supercell.get_size(); c++) {
    ...
    find_all_atom_in_supercell_ordered(&(mc(c).str.atom_pos),
                                       ...
                                       lat.atom_type, lat.cell, mc(c).str.cell);
    ...
    for (int at = 0; at < mc(c).str.atom_pos.get_size();) {
      ...
      for (int t = 0; t < mc(c).nbcomp(at); t++) {
        ...
        // cerr << rnum << " " << inum << endl;
        if (!near_zero((Real)inum - rnum))
          ERRORQUIT("Impossible to match point correlations due to "
                    "incompatible supercell size.");
    ...
    mc(c).obj = calc_objective_func(mc(c).corr, tcorr, mysqstol, weightdist,
                                    weightnbpt, weightdecay, diam, nbpt);
  ...
  while (1) {
    if (obj < best.obj) {
      ...
      write_structure(best.str, ulat, site_type_list, atom_label, axes,
                      strfile);
      ofstream corrfile;
      ...
      for (int t = 0; t < best.corr.get_size(); t++) {
        corrfile << nbpt(t) << "\t" << diam(t) << "\t" << best.corr(t) << "\t"
                 << tcorr(t) << "\t" << (best.corr(t) - tcorr(t)) << endl;
      }
    ...
    if (tic == 0) {
      ...
      if (tempfile) {
        ...
        if (newweightdist != weightdist || newweightnbpt != weightnbpt ||
            newweightdecay != weightdecay || newT != T) {
          logfile << "New parameters read in: -wr=" << newweightdist
                  << " -wp=" << newweightnbpt << " -wd=" << newweightdecay
                  << " -T=" << newT << endl;
        }
        T = newT;
        if (newweightdist != weightdist || newweightnbpt != weightnbpt ||
            newweightdecay != weightdecay) {
          ...
          for (int c = 0; c < mc.get_size(); c++) {
            mc(c).obj =
                calc_objective_func(mc(c).corr, tcorr, mysqstol, weightdist,
                                    weightnbpt, weightdecay, diam, nbpt);
            if (mc(c).obj < best.obj) {
              best = mc(c);
              cc = c;
    ...
    Real newobj = calc_objective_func(newcorr, tcorr, mysqstol, weightdist,
                                      weightnbpt, weightdecay, diam, nbpt);
