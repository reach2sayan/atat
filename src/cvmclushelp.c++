const char *helpstring =
    ""
    "This code generates all the input coefficients needed to construct the "
    "CVM (Cluster Variation Method) entropy expression.\n"
    "\n"
    ...
    "       (including possible functions on each sites for systems beyond "
    "binary).\n"
    "\n"
    "It takes as an input\n"
    "(1) a lattice (by default in lat.in) in a format identical to (m)maps. "
    "(See corrdump -h for a description.)\n"
    "(2) a maximal cluster (by default in maxclus.in) defined by a list of "
    "sites, expressed in the coordinate system defined by the first lines of "
    "lat.in\n"
    "\n"
    "It produces as an output:\n"
    "(1) clusters.out, the list of all symmetrically distinct subclusters of "
    "the maximal cluster,\n"
    "     in the standard ATAT format (See corrdump -h for a description.)\n"
    "     These include not only the set of sites but also, for systems beyond "
    "binaries, possible functions\n"
    "     on each sites of the subclusters, so one set of sites may appear "
    "more than once.\n"
    "\n"
    "(2) clusmult.out, the multiplicities (per unit cell) for each clusters "
    "given in (i).\n"
    "    The file starts with the number of clusters and then lists their "
    "multiplicities.\n"
    "    (That information in in (i) too, but they are separated out here for "
    "convenience.\n"
    "    This is not needed for the CVM entropy, only for the energy "
    "expression:\n"
    "        E = sum_q J_q mu_q xi_q\n"
    "      where q is a cluster with functions (for beyond binary)\n"
    "            J_q are the Effective Cluster Interations (not provided by "
    "this code)\n"
    ...
    "    (Note that there may be fewer clusters than in clusters.out because, "
    ...
    "        [coordinates of the site] [occupation: as a integer in "
    "{0,...,k-1}] [k]\n"
    "     (indentation is for clarity - the actual file is not "
    "indented.)	\n"
    "\n"
    "(4)  vmat.out, the `v-matrix'. There is one v-matrix per cluster and\n"
    "     each v-matrix has different correlations along columns and different "
    "configurations along rows\n"
    "     the order is the same as in clusters.out and config.out. Format:\n"
    "     [nb of clusters n]\n"
    "     repeated for each of the n clusters:\n"
    "      [number of rows (config)] [number of columns (clusters)]\n"
    "      [vmatrix, each column separated by space and each row separated by "
    "newlines]\n"
    "\n"
    "(5)  configcoef.out, the multiplicities and Kikuchi-Barker coefficients. "
    "Format:\n"
