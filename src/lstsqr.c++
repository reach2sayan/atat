...
void calc_ols(Array<Real> *pb, const Array2d<Real> &x, const Array<Real> &y,
              const Array<Real> &weight, int nonredundant) {
...
int build_basis(Array<ArrayReal> *pbasis, int *pnb_in_basis,
  ...
  if (nb_in_basis == 0)
    basis.resize(new_vector.get_size());
...
void gram_schmidt(Array<Array<Real>> *portho_basis,
  ...
  for (int i = 0; i < basis.get_size(); i++) {
    ...
    product(&((*portho_basis)(i)), basis(i),
            1. / sqrt(inner_product((*portho_basis)(i), (*portho_basis)(i))));
  }
}
