...
ClusterBank::ClusterBank(const rMatrix3d &cell,
                         const Array<rVector3d> &atom_pos, int ntuple,
                         const SpaceGroup &_equivalent_by_symmetry)
    : cluster_list(), current_cluster(), current_index(0), previous_length(0.),
      multiplet(cell, atom_pos, ntuple),
  ...
  if (!current_cluster && save->get_size() > 1) {
    do {
      multiplet++;
        ...
        !add_unique(&cluster_list, Cluster(multiplet), equivalent_by_symmetry));
    current_cluster = save;
    current_cluster++;
  }
}

MultiClusterBank::MultiClusterBank(const Structure &_lat, int ntuple,
                                   const SpaceGroup &_equivalent_by_symmetry)
    : lat(_lat),
      cluster_bank(_lat.cell, _lat.atom_pos, ntuple, _equivalent_by_symmetry),
      cluster_list(), current_cluster(), current_index(0), previous_length(0.),
      equivalent_by_symmetry(_equivalent_by_symmetry) {
  if (ntuple > 1) {
    make_new_clusters();
...
MultiClusterBank::MultiClusterBank(const MultiClusterBank &bank, int ntuple)
    : lat(bank.lat), cluster_bank(bank.lat.cell, bank.lat.atom_pos, ntuple,
                                  bank.equivalent_by_symmetry),
      cluster_list(), current_cluster(), current_index(0), previous_length(0.),
...
void MultiClusterBank::make_new_clusters(void) {
  ...
  for (int at = 0; at < new_clus.clus.get_size(); at++) {
    new_clus.site_type(at) =
        lat.atom_type(which_atom(lat.atom_pos, new_clus.clus(at), inv_cell)) -
        2;
  }
