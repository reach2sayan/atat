char *helpstring =
    ...
    "-The first symbol listed is assigned a spin of -1 and the second, a spin "
    "of 1.\n"
    ...
    "\n"
    "  Contains the reference energy (per site) to be subtracted to get "
    "formation energies.\n"
    "  The first line is the c=0 energy, the second line is the c=1 energy.\n"
    "  If this file is omitted, the energies of leftmost and rightmost "
    "structures\n"
    "  (on the concentration axis) are taken.\n"
    "\n"
    "Optional input file: nbclusters.in\n"
    "  Allows the user to manually select which clusters to include in the "
    "fit.\n"
    ...
    "\n"
    "These warning should disappear as more structural energies become "
    "available\n"
    "and the following messages should be displayed:\n"
    "  'Among structures of known energy, true and predicted ground states "
    "agree.'\n"
    ...
    "(before the weighting is turned on in order to get the correct ground "
    "states).\n"
    "\n"
    "fit.out\n"
    ...
    "'energy' is per site (a site is a place where more than one atom type can "
    "lie)\n"
    "'weight' is the weight of this structure in the fit.\n"
    "'index' is the name of the directory associated with this structure.\n"
    "\n"
    "predstr.out\n"
    "Contains the predicted energy (per site) of all structures maps has in "
    "memory but\n"
    "whose true energy is unknown or has been flagged with error.\n"
    "Format: one structure per line, and each line has the following "
    "information:\n"
    ...
    "status is either b for busy (being calculated), e for error or u for "
    "unknown\n"
    "(not yet calculated). A g is appended to status if that structure is "
    "predicted to be\n"
    ...
    "\n"
    "gs_str.out\n"
    "Lists the ground state structures, in the same format as the n/str.out "
    "files\n"
    "(see below). Each structure is terminated by the word 'end' on a line by "
    "itself,\n"
    ...
    "\n"
    "clusters.out\n"
    "For each cluster, the first line is the multiplicity, the second line is "
    "the\n"
    "cluster diameter, and the third line is the number of points in the "
    "cluster.\n"
    "The remaining lines are the coordinates of the points in the cluster\n"
    "(in the coordinate system specified in the input file defining the "
    "lattice).\n"
    ...
    "\n"
    "Unless otherwise specified all files mentioned reside in the directory "
    "where\n"
    "maps was started. All paths are relative to the startup directory.\n"
    "\n"
    "+The script should first wait for computer time to be available before "
    "creating\n"
    " a file called 'ready'.\n"
    "-Upon detecting that the 'ready' file has been created,\n"
    " maps responds by creating a subdirectory 'n' (where 'n' is a number) and "
    "a file\n"
    " 'n/str.out' containing a description of a structure whose energy needs "
    "to be\n"
    " calculated.\n"
    ...
    " the script should now look for the 'n/wait' file, start the "
    "calculations\n"
    " in the directory 'n' and delete file 'n/wait'.\n"
    "+If anything goes wrong in the calculations, the script should create a "
    "file\n"
    " 'n/error'.\n"
    "+When the calculations terminate successfully, the energy per unit cell "
    "of the structure\n"
    " should be copied to the file 'n/energy'.\n"
    " (NOTE: use energy per unit cell of the structure NOT per unit cell of "
    "the lattice).\n"
    "-maps continuously scans all the subdirectories 'n' for 'n/energy' or "
    "'n/error'\n"
    " files and updates the cluster expansion accordingly.\n"
    "-maps updates the cluster expansion whenever a file called 'refresh' is "
    "created\n"
    " (maps then deletes it).\n"
    "-maps terminates when a 'stop' file is created.\n"
    "\n"
    "Note that the script can ask maps to create new structure directories "
    "even before\n"
    ...
    "    /* in background: run code and create either energy file or error "
    "file */\n"
    "    cd ..\n"
    "  endif\n"
    "  sleep 180\n"
    ...
    "1) converts 'vasp.wrap' and 'n/str.out' into all the necessary files to "
    "run vasp,\n"
    "2) runs vasp\n"
    "3) extract all the information from the output files and writes in a "
    "format\n"
    "   readable by maps.\n"
    "\n"
